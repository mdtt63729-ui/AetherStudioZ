...
class JetsnackPreviewProfileTest {
    ...
    private fun profileEntry(root: Path, lowering: KotlinPreviewLowering, rel: String, entryKey: String) {
        ...
        val sink = dev.aetherstudioz.platform.log.LogSink { r ->
            if (r.tag == "kotlin-perf" && "total=" in r.message) say("    [perf] ${r.message}")
        }
        dev.aetherstudioz.platform.log.Log.addSink(sink)
        dev.aetherstudioz.platform.log.PerfTrace.enabled = true
        val (seed, model) = dev.aetherstudioz.lang.kotlin.KotlinPerf.trace("bench.cold") {
        ...
        dev.aetherstudioz.platform.log.PerfTrace.enabled = false
        dev.aetherstudioz.platform.log.Log.removeSink(sink)
        ...
        model.program[entryKey]?.let { e ->
            val rf = dev.aetherstudioz.lang.kotlin.interp.reachableSourceFunctions(e, model.program, model.classes)
            val rc = dev.aetherstudioz.lang.kotlin.interp.reachableSourceClasses(e, model.program, model.classes)
