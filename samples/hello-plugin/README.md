...
A AetherStudioZ plugin shipped as its own Android app. The IDE finds it through the package manager, reads its
...
| Part | Where |
| --- | --- |
| The packaged manifest, as TOML | [`res/raw/aetherstudioz_plugin.toml`](src/main/res/raw/aetherstudioz_plugin.toml) |
| A marker activity carrying `dev.aetherstudioz.aetherstudioz.action.PLUGIN` and pointing `meta-data` at that resource | [`AndroidManifest.xml`](src/main/AndroidManifest.xml) |
...
Then restart AetherStudioZ and open **Settings > Plugins > Installed**. The plugin loads at startup, so a
...
```kotlin
dependencies {
    compileOnly("io.github.aetherstudioz:plugin-api:1.2.0")
    compileOnly("io.github.aetherstudioz:platform-core:1.2.0")

    // The build facet. It brings project-model-api, vfs-api and platform-core with it.
    compileOnly("io.github.aetherstudioz:build-api:1.2.0")
    ...
    compileOnly("io.github.aetherstudioz:plugin-ui-api:1.2.0")
...
The Compose *compiler* needs no declaration when the project is built by AetherStudioZ: it applies the plugin to
...
- Your plugin's own license is yours to choose. The published SPI modules are GPL-3.0-or-later **with** the
  ...
  make your plugin a derivative work of AetherStudioZ.
