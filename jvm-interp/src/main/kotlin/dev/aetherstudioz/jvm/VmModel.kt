...
internal class VmLambda(
    ...
    fun invokeSamReal(samArgs: List<Any?>): Any? =
        try {
            ...
            // `dev.aetherstudioz.jvm.VmException` FATAL on the caller's thread, e.g. a Compose measure/effect lambda).
 ...
 * peer. Its class is `dev.aetherstudioz.jvm.peers.…_Peer3`, so a `catch` naming the interpreted type would match nothing
