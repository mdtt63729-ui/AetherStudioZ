...
import dev.aetherstudioz.interp.InterpretedLambda
import dev.aetherstudioz.interp.compose.ComposeDispatcher
 ...
 *       -Pandroid.testInstrumentationRunnerArguments.class=dev.aetherstudioz.android.spike.VmMaterial3PerfArtSpike
...
class VmMaterial3PerfArtSpike {
    ...
    fun material3InterpretVsReflectPerfOnArt() {
        ...
        fun oneColdButton(diskDir: java.nio.file.Path? = null): Triple<Double, Double, Double> {
            ...
            dev.aetherstudioz.jvm.VmProfile.reset(); DexPeerFactory.reset()
            var ms = 0.0
            ActivityScenario.launch(SpikeComposeActivity::class.java).use { scenario ->
                scenario.onActivity { activity ->
                    activity.setContent {
            ...
            return Triple(ms, dev.aetherstudioz.jvm.VmProfile.parseNanos.get() / 1_000_000.0, DexPeerFactory.dexNanos.get() / 1_000_000.0)
        }
        ...
        val (mc, mw) = measure(interpreted, { k -> noArgCall("dev.aetherstudioz.interp.compose.jetpacksamples.JetpackSamplesKt", "MiniScreenSample", k) }, { emptyList() })
