...
import dev.aetherstudioz.android.ArtKotlinPluginLoader
import dev.aetherstudioz.build.SourceGenRequest
import dev.aetherstudioz.ksp.BundledKspProcessors
import dev.aetherstudioz.ksp.BundledKspThin
 ...
 * the app and already ART-patched by the `dev.aetherstudioz.kotlinc-art` ASM passes). So the `ksp.*`-relocated-platform
 ...
 * reflectively — exactly as the production `dev.aetherstudioz.ksp.KspSourceGenerator` does it.
 *
 ...
 *       -Pandroid.testInstrumentationRunnerArguments.class=dev.aetherstudioz.android.spike.KspArtSpikeTest
        ...
        val exitName: String = try {
            ...
            fail(
                "KSP2 thin runner failed to run on ART: ${t.javaClass.name}: ${t.message}\n" +
                    ...
                    "class, add/extend a pass in dev.aetherstudioz.build.kotlinc.ArtPatchPasses; if it is a " +
