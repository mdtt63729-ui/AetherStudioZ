package dev.aetherstudioz.android.bench

import android.content.Context
import android.util.Log
import androidx.test.ext.junit.runners.AndroidJUnit4
import androidx.test.platform.app.InstrumentationRegistry
import dev.aetherstudioz.android.ForkedD8Dexer
...
import dev.aetherstudioz.deps.ArtifactKind
import dev.aetherstudioz.deps.ConflictPolicy
import dev.aetherstudioz.deps.Repository
import dev.aetherstudioz.deps.ResolvedArtifact
import dev.aetherstudioz.deps.impl.MavenDependencyResolver
import dev.aetherstudioz.deps.impl.ResolverCache
...
import dev.aetherstudioz.platform.ProgressReporter
 ...
 *       -Pandroid.testInstrumentationRunnerArguments.class=dev.aetherstudioz.android.bench.OnDeviceBuildBenchmarkTest
 *     adb logcat -s BuildBench
 *
 * Instrumentation args (all optional):
 *   -e project   <name>        which project under aetherstudioz/projects to build (default: first android-app found)
...
class OnDeviceBuildBenchmarkTest {
    ...
    fun freshBuildOfAnOnDeviceProject() {
        ...
        // The app's on-device home (external app storage): aetherstudioz/projects/<name>, plus the shared caches.
        val home = File(ctx.getExternalFilesDir(null) ?: ctx.filesDir, "aetherstudioz")
        ...
        // Leave a report the user can pull: adb pull .../files/aetherstudioz/build-bench-report.txt
     ...
     *
     *     ./gradlew :ide-android:connectedDebugAndroidTest \
     *       -Pandroid.testInstrumentationRunnerArguments.class=dev.aetherstudioz.android.bench.OnDeviceBuildBenchmarkTest#freshComposeBuild
     *     adb logcat -s BuildBench
     *
     * Instrumentation args (all optional):
     ...
     * AetherStudioZ project with the tolerant Gradle importer ([GradleImport]), resolves its declared dependency
     ...
     *
     *     adb push <checkout>/Jetsnack /sdcard/Android/data/com.aetherstudioz.app/files/jetsnack-src
     *     ./gradlew :ide-android:connectedDebugAndroidTest \
     *       -Pandroid.testInstrumentationRunnerArguments.class=dev.aetherstudioz.android.bench.OnDeviceBuildBenchmarkTest#assembleImportedGradleProject
     *     adb logcat -s BuildBench
     */
    @Test
    fun assembleImportedGradleProject() {
        ...
        try {
            ...
            val coords = app.dependencies.filterIsInstance<dev.aetherstudioz.model.LibraryDependency>()
                .filter { it.scope.onCompile }
                .mapNotNull { coordinateOf(it.library.name) }
            val boms = app.dependencies.filterIsInstance<dev.aetherstudioz.model.PlatformDependency>().map { it.bom }
            ...
            project.beginModification().apply {
                ...
                app.dependencies.filter { it is LibraryDependency || it is dev.aetherstudioz.model.PlatformDependency }
            ...
            val ksp = dev.aetherstudioz.ksp.KspSourceGenerator(
                runnerClasspath = { listOfNotNull(dev.aetherstudioz.ksp.BundledKspThin.jar()) },
                processors = { req -> dev.aetherstudioz.ksp.KspProcessorCatalog.bundled().classpathFor(req.classpath, req.declaredDependencies) },
                loader = dev.aetherstudioz.ksp.KspProcessorLoader { cp ->
                    dev.aetherstudioz.android.ArtKotlinPluginLoader(androidJar, File(work, "ksp-loader-cache").toPath(), minApi = 26).load(cp)
                },
                jdkHome = null,
     ...
     * `dev.aetherstudioz.core.DependencyPartition` (internal to ide-core, so unavailable from this androidTest module).
