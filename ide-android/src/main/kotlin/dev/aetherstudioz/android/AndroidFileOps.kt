...
internal class AndroidFileOps(private val activity: ComponentActivity) {
    ...
    /**
     * Open the system Files app on AetherStudioZ's storage (served by `ProjectsDocumentsProvider`) so the user
    ...
    fun openInFiles(path: String?) {
        ...
        runCatching { activity.startActivity(browse) }.getOrElse {
            Toast.makeText(activity, "Open the Files app → AetherStudioZ to browse your projects", Toast.LENGTH_LONG).show()
        }
    }
