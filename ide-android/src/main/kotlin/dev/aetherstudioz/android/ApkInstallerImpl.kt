...
import dev.aetherstudioz.android.daemon.PackageLaunchBridge
import dev.aetherstudioz.core.ApkInstaller
...
class ApkInstallerImpl(context: Context) : ApkInstaller {
    ...
    override suspend fun installAndLaunch(apk: Path, packageName: String, log: (String) -> Unit): Boolean = withContext(Dispatchers.IO) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && !pm.canRequestPackageInstalls()) {
            log("Allow AetherStudioZ to install apps (Settings → Install unknown apps), then Run again.")
            runCatching {
    ...
    private companion object {
        const val INSTALL_ACTION = "dev.aetherstudioz.android.INSTALL_STATUS"
    }
}
