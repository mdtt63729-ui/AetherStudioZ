...
import dev.aetherstudioz.android.AppLogSinkRegistry
...
class BuildDaemonService : Service() {
     ...
     *  LocalServerSocket the debug app connects to; [dev.aetherstudioz.core.IdeServices.appLogState] carries the ring
     *  buffer + a monotonic [dev.aetherstudioz.core.AppLogSnapshot.totalAppended] so we forward only the lines the UI
        ...
        if (nm != null && nm.getNotificationChannel(CHANNEL_ID) == null) {
            nm.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Builds", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Shown while AetherStudioZ is building or running your project"
                },
            )
        }
     ...
     * for android.jar and every library, held by [dev.aetherstudioz.android.support.tools.SharedDexClasspath] for the
    ...
    private data class NotifContent(
        ...
        companion object {
            val PREPARING = NotifContent("AetherStudioZ", "Preparing…", 0, 0, true)
        }
    }
