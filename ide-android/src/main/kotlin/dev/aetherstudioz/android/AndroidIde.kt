package dev.aetherstudioz.android
...
import dev.aetherstudioz.analytics.DeviceInfo
import dev.aetherstudioz.analytics.EventCategory
import dev.aetherstudioz.analytics.Events
import dev.aetherstudioz.android.fork.ForkedKotlinCompiler
import dev.aetherstudioz.android.fork.ProcessIdentity
import dev.aetherstudioz.android.plugins.ApkPluginSource
import dev.aetherstudioz.android.preview.SwingAwareProgramInterpreter
...
import dev.aetherstudioz.analytics.impl.AnalyticsLogSink
import dev.aetherstudioz.analytics.impl.DefaultAnalyticsService
import dev.aetherstudioz.analytics.impl.SupabaseSink
import dev.aetherstudioz.core.ANALYTICS_SERVICE
import dev.aetherstudioz.core.NOTIFICATION_PRESENTER
import dev.aetherstudioz.core.STORE_ACCOUNT_SERVICE
import dev.aetherstudioz.core.STORE_CATALOG_SOURCE
import dev.aetherstudioz.core.STORE_REVIEW_SERVICE
import dev.aetherstudioz.core.STORE_SUBMISSION_SERVICE
...
import dev.aetherstudioz.platform.log.Log.addSink
import java.io.File
 ...
 * directory lives under `<external-files>/aetherstudioz` (projects in `projects/`, one workspace dir each) —
...
object AndroidIde {
    ...
    fun bootstrap(context: Context): Session {
        ...
        dev.aetherstudioz.platform.RuntimeInfo.set32Bit(!android.os.Process.is64Bit())

        // App-specific EXTERNAL storage (Android/data/<pkg>/files/aetherstudioz)
        ...
        dev.aetherstudioz.platform.EngineBreadcrumb.init(crumbFile.toPath())
        reportPreviousNativeCrash(context, manager, analytics, dev.aetherstudioz.platform.EngineBreadcrumb.readLast())
        ...
        val storeAccounts: dev.aetherstudioz.store.StoreAccountService =
            supabaseAccounts ?: dev.aetherstudioz.store.StoreAccountService.Unsupported
        ...
        val backend = IdeServicesBackend(
            ...
            buildRunnerFactory = { svc ->
                dev.aetherstudioz.android.daemon.RemoteBuildRunner(
                    appContext, svc
                )
            },
        ...
        // Notifications that arrived by push while no engine existed (see AetherStudioZMessagingService).
        ...
        if (backend.diagnostics.analyticsConsent() == true) {
            backend.diagnostics.track(
                dev.aetherstudioz.analytics.Events.COLD_START,
    ...
    fun createProjectManager(context: Context): ProjectManager {
        ...
        // PathUtil reads (see build-logic dev.aetherstudioz.build.kotlinc.PathUtilSelfLocatePass). Without this, the
        ...
        val previewRuntime: dev.aetherstudioz.preview.impl.CustomViewRuntime? = null
        ...
        // Kotlin compilation runs in a PERSISTENT forked VM (`dev.aetherstudioz.android.fork`): a compile then gets a
        ...
        val realViewRuntime = dev.aetherstudioz.preview.realview.RemoteRealViewRuntime(
            context.applicationContext,
            androidJar.toPath(),
        ...
        // Project data left by previous app versions (same `com.aetherstudioz.app` package, so the same external
        ...
        //  - `filesDir/aetherstudioz` — an early internal-storage home of THIS app, before the move to external.
        ...
        val legacyInternalHome = File(context.filesDir, "aetherstudioz").toPath()
        // When external storage is unusable, [externalHome] falls back to internal `filesDir`, so the ACTIVE
        // home (`home`, above) IS `filesDir/aetherstudioz` = legacyInternalHome. Never list the active home as a
    ...
    /**
     * If a PREVIOUS process died of a native crash (the ART SIGSEGV; see [dev.aetherstudioz.platform.RuntimeInfo]; seen on
     ...
     *  - the OS tombstone ([dev.aetherstudioz.platform.NativeTombstone]), which names the thread that actually faulted,
     ...
     * ([dev.aetherstudioz.platform.ForkedToolVm.isToolVmCrash]). Those forks are how the app measures the heap a device
    ...
    private fun reportPreviousNativeCrash(
        ...
        analytics: dev.aetherstudioz.analytics.AnalyticsService,
        prev: dev.aetherstudioz.platform.EngineBreadcrumb.Crumb?,
    ) {
        ...
        runCatching {
            ...
            val reportable = ArrayList<Pair<android.app.ApplicationExitInfo, dev.aetherstudioz.platform.NativeTombstone?>>()
            for (exit in candidates) {
                if (reportable.size >= NATIVE_CRASH_MAX_REPORTED) break
                val tomb = runCatching {
                    exit.traceInputStream?.use { dev.aetherstudioz.platform.NativeTombstone.parse(it) }
                }.getOrNull()
                if (dev.aetherstudioz.platform.ForkedToolVm.isToolVmCrash(tomb)) {
            ...
            com.google.firebase.messaging.FirebaseMessaging.getInstance().token
                .addOnSuccessListener { token ->
                    ...
                    Thread({
                        ...
                        if (result is dev.aetherstudioz.store.StoreResult.Ok) {
    ...
    private fun buildStoreAccounts(): dev.aetherstudioz.store.impl.SupabaseAccountService? {
        val url = BuildConfig.SUPABASE_URL
        val key = BuildConfig.SUPABASE_KEY
        if (url.isBlank() || key.isBlank()) return null
        return dev.aetherstudioz.store.impl.SupabaseAccountService(
            url = url,
            apiKey = key,
            // The deep link the manifest's intent-filter receives.
            redirectUrl = dev.aetherstudioz.store.StoreAuth.ANDROID_REDIRECT,
            enabledProviders = listOf(
                dev.aetherstudioz.store.StoreProvider.GITHUB,
                dev.aetherstudioz.store.StoreProvider.GOOGLE,
            ),
        )
    }

    private fun buildStoreReviews(
        accounts: dev.aetherstudioz.store.impl.SupabaseAccountService?,
    ): dev.aetherstudioz.store.StoreReviewService {
        val url = BuildConfig.SUPABASE_URL
        val key = BuildConfig.SUPABASE_KEY
        if (url.isBlank() || key.isBlank()) return dev.aetherstudioz.store.StoreReviewService.Unsupported
        ...
        return dev.aetherstudioz.store.impl.SupabaseReviewService(url, key, accounts)
    }

    /** Submitting needs the live session, so it takes the concrete account service, not the port. */
    private fun buildStoreSubmissions(
        accounts: dev.aetherstudioz.store.impl.SupabaseAccountService?,
    ): dev.aetherstudioz.store.StoreSubmissionService {
        if (accounts == null) return dev.aetherstudioz.store.StoreSubmissionService.Unsupported
        return dev.aetherstudioz.store.impl.SupabaseSubmissionService(
    ...
    private fun buildStoreSource(): dev.aetherstudioz.store.StoreCatalogSource {
        val url = BuildConfig.SUPABASE_URL
        val key = BuildConfig.SUPABASE_KEY
        if (url.isBlank() || key.isBlank()) return dev.aetherstudioz.store.StoreCatalogSource.Unconfigured
        return dev.aetherstudioz.store.impl.SupabaseStoreSource(
            url = url,
            apiKey = key,
    ...
    private fun reportNativeExit(
        ...
        tomb: dev.aetherstudioz.platform.NativeTombstone?,
        analytics: dev.aetherstudioz.analytics.AnalyticsService,
        prev: dev.aetherstudioz.platform.EngineBreadcrumb.Crumb?,
    ) {
    ...
    ): dev.aetherstudioz.analytics.AnalyticsService {
        // One shared Supabase project; the analytics sink and the store catalog read the same pair.
        val url = BuildConfig.SUPABASE_URL
        val key = BuildConfig.SUPABASE_KEY
        if (url.isBlank() || key.isBlank()) return dev.aetherstudioz.analytics.NoopAnalyticsService
     ...
     * `ENOENT` (the `aetherstudioz/android.jar` startup crash). So don't just null-check: require the volume to be
    ...
    /** The whole AetherStudioZ app directory (`<external-files>/aetherstudioz`): projects, the SDK `android.jar`,
     *  the debug keystore, the kotlinc home, shared caches. This is the root surfaced to file managers. */
    fun appHomeDir(context: Context): File = File(externalHome(context), "aetherstudioz")

    /** The on-disk projects directory (`<external-files>/aetherstudioz/projects`). */
