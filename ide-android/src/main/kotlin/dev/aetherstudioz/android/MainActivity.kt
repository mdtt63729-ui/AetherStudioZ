 ...
 * shared Compose IDE UI ([AetherStudioZApp]) over the resulting [IdeBackend] — the same path the desktop runs.
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            when {
                ...
                else -> Splash("Starting AetherStudioZ…")
            }
        }
    }
     ...
     * `content` VIEW intents, and a stale `aetherstudioz://auth-callback` sitting in `getIntent()` would be
    ...
    private fun takeAuthRedirect(intent: Intent?): Boolean {
        ...
        if (!dev.aetherstudioz.store.StoreAuth.isAuthRedirect(url)) return false
