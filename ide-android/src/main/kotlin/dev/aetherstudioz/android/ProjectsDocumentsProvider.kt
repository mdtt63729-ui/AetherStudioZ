...
/**
 * Surfaces AetherStudioZ's whole external app directory (`getExternalFilesDir(null)`) to the system Files app
 * and any SAF-aware file manager as a browsable, **read/write** root — without the All-Files-Access
 * permission. The exposed tree is the entire external files dir: the new app home (`aetherstudioz/` with its
...
class ProjectsDocumentsProvider : DocumentsProvider() {
    ...
    override fun queryRoots(projection: Array<out String>?): Cursor {
        ...
        cursor.newRow().apply {
            ...
            add(Root.COLUMN_TITLE, "AetherStudioZ")
    ...
    private fun addFileRow(cursor: MatrixCursor, file: File) {
        ...
        cursor.newRow().apply {
            ...
            add(Document.COLUMN_DISPLAY_NAME, if (file.absolutePath == rootId) "AetherStudioZ" else file.name)
