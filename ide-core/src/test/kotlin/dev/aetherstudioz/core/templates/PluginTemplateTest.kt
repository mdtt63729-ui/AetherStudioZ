...
import dev.aetherstudioz.core.plugins.PluginManifestToml
...
class PluginTemplateTest {
    ...
    fun `the marker activity and its manifest resource are wired up`() {
        withTempDir("plugin-template-marker") { dir ->
            IdeServices.createProjectAt(
                ...
                assertTrue("dev.aetherstudioz.aetherstudioz.action.PLUGIN" in androidManifest, "missing marker action")
                assertTrue("dev.aetherstudioz.aetherstudioz.plugin.manifest" in androidManifest, "missing meta-data key")
                assertTrue("@raw/aetherstudioz_plugin" in androidManifest, "meta-data does not point at the manifest")
    ...
    fun `a panel plugin compiles against the UI SPI and the IDE's own Compose`() {
        val deps = AetherStudioZPluginTemplate.dependencies(TemplateArgs(args("contributes" to "panel")))
        val coordinates = deps.map { it.coordinate }
        assertTrue(
            "io.github.aetherstudioz:plugin-ui-api:$PLUGIN_SPI_VERSION" in coordinates,
    ...
    fun `the SPI is declared compileOnly at the published coordinates`() {
        val deps = AetherStudioZPluginTemplate.dependencies(TemplateArgs(args()))
        assertEquals(
            listOf(
                "io.github.aetherstudioz:plugin-api:$PLUGIN_SPI_VERSION",
                "io.github.aetherstudioz:platform-core:$PLUGIN_SPI_VERSION",
            ),
    ...
    private companion object {
        const val TEMPLATE = "aetherstudioz-plugin"
        const val NAME = "My Tool"
        const val PKG = "com.example.mytool"
        const val TOML = "plugin/src/main/res/raw/aetherstudioz_plugin.toml"
    }
}
