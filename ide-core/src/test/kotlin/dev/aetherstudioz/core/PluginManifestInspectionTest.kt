 ...
 *
 * A `aetherstudioz_plugin.toml` has no language of its own and lives under `res/raw`, which is not a source
...
class PluginManifestInspectionTest {
    ...
    fun `flags a missing marker activity, which would make the plugin invisible`() {
        ...
        assertTrue("dev.aetherstudioz.aetherstudioz.action.PLUGIN" in problem.message, problem.message)
    ...
    private fun write(services: IdeServices, text: String): Path {
        val file = root.resolve("app/src/main/res/raw/aetherstudioz_plugin.toml")
        Files.createDirectories(file.parent)
        Files.writeString(file, text)
        services.modules()
        return file
