...
import dev.aetherstudioz.store.RemoteCatalog
import dev.aetherstudioz.store.RemoteStoreItem
import dev.aetherstudioz.store.StoreCatalogSource
import dev.aetherstudioz.store.StoreQuery
import dev.aetherstudioz.store.StoreResult
import dev.aetherstudioz.store.impl.StoreFeedParser
...
class StoreFeedWiringTest {
    ...
    fun chartMovementSurvivesTheMapping() {
        ...
        val ui = StoreFeedMapper.toUi(feed, emptyMap())
        val charts = ui.sections.filterIsInstance<UiFeedSection.Charts>().single()
        val source = feed.sections.filterIsInstance<dev.aetherstudioz.store.StoreSection.Charts>().single()
