...
class AndroidXmlCompletionTest {
    ...
    fun completesCustomViewTagsFromLibrariesByFqnAndSimpleName() {
        val views = listOf(
            dev.aetherstudioz.android.support.metadata.Widget("com.google.android.material.button.MaterialButton", false),
            dev.aetherstudioz.android.support.metadata.Widget("androidx.constraintlayout.widget.ConstraintLayout", true),
        )
        val svc = XmlCompletion(contributors = {
    ...
    fun resourceReferenceShowsResolvedValueAsHint() {
        ...
        val svc = XmlCompletion(contributors = {
            listOf(AndroidXmlContributor(resources = { type ->
                if (type == ResourceType.STRING) listOf(ResourceCandidate("app_name", "AetherStudioZ"), ResourceCandidate("greeting"))
                else emptyList()
            }))
        })
        val items = completeItems("<TextView android:text=\"@string/|\"", svc = svc)
        // The resolved value is the popup hint…
        assertEquals("AetherStudioZ", items.first { it.label == "@string/app_name" }.detail)
