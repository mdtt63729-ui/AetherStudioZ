...
class PluginManifestCompletionTest {
    ...
    private fun complete(
        text: String,
        caretAfter: String,
        fileName: String = "aetherstudioz_plugin.toml",
    ...
    private class FakeIndex(private val pluginClasses: List<String>, ready: Boolean) : IndexService {
        ...
        override fun <V : Any> exact(id: IndexId, key: String): Sequence<V> {
            if (id !in SubtypeIndex.ALL || key != SubtypeIndex.key("dev.aetherstudioz.plugin.Plugin")) return emptySequence()
            ...
            return pluginClasses
                .map { SubtypeValue(fqn = it, kind = "class", supertype = "dev.aetherstudioz.plugin.Plugin") as V }
                .asSequence()
        }

        override fun <V : Any> prefix(id: IndexId, prefix: String, limit: Int) = emptySequence<dev.aetherstudioz.index.Hit<V>>()
        override fun <V : Any> fuzzy(id: IndexId, pattern: String, limit: Int) = emptySequence<dev.aetherstudioz.index.Hit<V>>()
    ...
    private class CollectingResultSet(override val params: CompletionParams) : dev.aetherstudioz.lang.completion.CompletionResultSet {
        private val items = ArrayList<dev.aetherstudioz.lang.completion.CompletionItem>()
        override val prefix get() = params.prefix
        override val elements: List<dev.aetherstudioz.lang.completion.CompletionItem> get() = items
        override fun addElement(item: dev.aetherstudioz.lang.completion.CompletionItem) { items.add(item) }
        override fun addAllElements(items: Iterable<dev.aetherstudioz.lang.completion.CompletionItem>) { this.items.addAll(items) }
        override fun removeIf(predicate: (dev.aetherstudioz.lang.completion.CompletionItem) -> Boolean) { items.removeAll(predicate) }
        override fun replaceAll(transform: (dev.aetherstudioz.lang.completion.CompletionItem) -> dev.aetherstudioz.lang.completion.CompletionItem) {
