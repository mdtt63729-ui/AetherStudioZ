...
import dev.aetherstudioz.plugin.action.ActionContext
import dev.aetherstudioz.plugin.action.ActionEffect
import dev.aetherstudioz.plugin.action.ActionPlace
import dev.aetherstudioz.plugin.action.ActionPlaces
import dev.aetherstudioz.plugin.action.CaretContext
import dev.aetherstudioz.plugin.action.TextEdit
...
class EditorTextActionsTest {
    ...
    fun theLinesGroupNestsEveryActionAsASubmenu() {
        ...
        val submenu = menu.filterIsInstance<dev.aetherstudioz.plugin.impl.ResolvedMenuItem.Submenu>().single()
        assertEquals("Lines", submenu.text)
        val ids = submenu.items.filterIsInstance<dev.aetherstudioz.plugin.impl.ResolvedMenuItem.Action>().map { it.action.id }
        assertEquals(
            listOf(
...
class ExtractFileActionTest {
    ...
    private val manager = ActionManager(
        dev.aetherstudioz.platform.impl.ExtensionRegistryImpl().also { dev.aetherstudioz.core.actions.ExtractFileAction.register(it) },
    )
    ...
    private fun onDeclaration(offset: Int, start: Int, end: Int, kind: String = "class_decl") = CaretContext(
        ...
        nodeStart = start,
        nodeEnd = end,
        ancestors = listOf(dev.aetherstudioz.plugin.action.CaretAncestor("compilation_unit", 0, end)),
    )
    ...
    fun aMemberIsNotOfferedAsItsOwnFile() {
        ...
        val caret = CaretContext(
            ...
            ancestors = listOf(
                dev.aetherstudioz.plugin.action.CaretAncestor("class_decl", text.indexOf("class Helper"), text.length - 1),
                dev.aetherstudioz.plugin.action.CaretAncestor("compilation_unit", 0, text.length),
            ),
        )
