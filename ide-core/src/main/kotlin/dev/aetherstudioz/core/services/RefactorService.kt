...
import dev.aetherstudioz.core.RenameInfo
import dev.aetherstudioz.core.RenameOutcome
...
import dev.aetherstudioz.lang.java.JavaSourceAnalyzer
...
/**
 * WORKSPACE-scoped engine service: the Java rename refactoring, carved out of [dev.aetherstudioz.core.IdeServices].
    ...
    private fun renameRefsIn(module: Module, file: Path, text: String, token: Any): List<TextRange> {
        ...
        return when (val a = ctx.analyzerFor(module, LanguageId("java"))) {
            is JdtSourceAnalyzer ->
                (token as? dev.aetherstudioz.lang.jdt.rename.RenameTarget)?.let { JdtRename.referencesIn(a.parse(vf, text), it) } ?: emptyList()
            is JavaSourceAnalyzer ->
                (token as? dev.aetherstudioz.lang.java.rename.JavaRenameTarget)?.let { a.renameReferencesIn(name, text, it) } ?: emptyList()
            else -> emptyList()
        }
    }
