...
import dev.aetherstudioz.core.LoweredComposePreview
import dev.aetherstudioz.core.settings.BuiltInSettingsPages
import dev.aetherstudioz.core.LoweredPreviewParameter
import dev.aetherstudioz.core.PreviewRunResult
 ...
 * [dev.aetherstudioz.core.IdeServices] — lower a `@Preview` composable (cross-file + cross-module) to a renderable tree,
...
internal class ComposePreviewService(private val ctx: EngineContext) {
    ...
    private fun previewModelFor(
    ...
    ): dev.aetherstudioz.lang.kotlin.interp.PreviewModel? {
        ...
        fun lazyByOwner(pf: dev.aetherstudioz.lang.kotlin.symbols.KotlinSymbolService.PreviewSourceFile): dev.aetherstudioz.lang.kotlin.interp.PreviewLazyFile? {
        ...
        val provider = object : dev.aetherstudioz.lang.kotlin.interp.LazyPreviewDeclProvider {
            override fun fileDeclaringType(fqn: String): dev.aetherstudioz.lang.kotlin.interp.PreviewLazyFile? =
                kotlinModules.firstNotNullOfOrNull { (_, a, _) -> a.findDeclaringTypeFile(fqn) }
                    ?.let(::lazyByOwner)

            override fun filesDeclaringFunction(name: String): List<dev.aetherstudioz.lang.kotlin.interp.PreviewLazyFile> =
    ...
    fun composePreviewDiagnostics(
        ...
        val reachable = dev.aetherstudioz.lang.kotlin.interp.reachableSourceClasses(entry, program, classes)
        ...
        val fnProblems = dev.aetherstudioz.lang.kotlin.interp.reachableSourceFunctions(entry, program, classes)
    ...
    fun lowerComposePreview(
    ...
    ): LoweredComposePreview? = dev.aetherstudioz.lang.kotlin.KotlinPerf.trace("kt.lowerPreview") {
        val module = ctx.moduleForEditableFile(file) ?: return null
        val analyzer = ctx.analyzerFor(module, KotlinLanguageBackend.LANGUAGE_ID) as? KotlinSourceAnalyzer
            ?: return null
        val vf = ctx.store.vfs.fileFor(file)
        dev.aetherstudioz.lang.kotlin.KotlinPerf.span("parse") { ctx.refreshParse(analyzer, file, text) }
        ...
        if (strict && dev.aetherstudioz.lang.kotlin.interp.reachableSourceFunctions(entry, program, classes).any { !it.isComplete }) return null
