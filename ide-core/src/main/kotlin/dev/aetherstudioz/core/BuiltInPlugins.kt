...
import dev.aetherstudioz.agent.ui.AgentUiPlugin
import dev.aetherstudioz.analysis.ACTION_PROVIDER_EP
import dev.aetherstudioz.analysis.ANALYZER_EP
import dev.aetherstudioz.analysis.MODULE_ANALYSIS
import dev.aetherstudioz.android.support.ANDROID_RESOURCE_REPOSITORY
import dev.aetherstudioz.android.support.AndroidAidlProvider
...
import dev.aetherstudioz.android.support.ResourceRepositorySource
...
import dev.aetherstudioz.build.BUILD_CONTROL
...
import dev.aetherstudioz.core.actions.EditorTextActions
import dev.aetherstudioz.core.actions.ExtractFileAction
import dev.aetherstudioz.core.analysis.AidlAnalyzer
import dev.aetherstudioz.core.analysis.PackageMismatchAnalyzer
import dev.aetherstudioz.core.analysis.PluginManifestAnalyzer
import dev.aetherstudioz.core.completion.BufferWordsContributor
import dev.aetherstudioz.core.completion.CompletionStats
import dev.aetherstudioz.core.completion.PluginManifestCompletion
import dev.aetherstudioz.core.completion.PostfixContributor
import dev.aetherstudioz.core.completion.UserLiveTemplateContributor
import dev.aetherstudioz.core.gradle.GradleBuildFileWriter
import dev.aetherstudioz.core.gradle.GradleProjectImporter
...
import dev.aetherstudioz.core.templates.AetherStudioZPluginTemplate
...
import dev.aetherstudioz.core.templates.SwingAppTemplate
import dev.aetherstudioz.core.templates.SwingCanvasTemplate
import dev.aetherstudioz.core.templates.WeatherSampleTemplate
import dev.aetherstudioz.index.INDEX_EP
import dev.aetherstudioz.index.SYMBOL_SEARCH
import dev.aetherstudioz.ksp.DefaultKspProcessorLoader
import dev.aetherstudioz.ksp.KspProcessorCatalog
import dev.aetherstudioz.ksp.KspProcessorLoader
import dev.aetherstudioz.ksp.KspSourceGenerator
...
import dev.aetherstudioz.lang.completion.StatsWeigher
import dev.aetherstudioz.lang.java.JavaLanguageBackend
...
import dev.aetherstudioz.lang.jdt.analysis.JdtAnalysisSupport
import dev.aetherstudioz.lang.kotlin.KotlinLanguageBackend
import dev.aetherstudioz.lang.kotlin.analysis.KotlinAnalysisSupport
...
import dev.aetherstudioz.lang.kotlin.index.KotlinClassNamesIndex
import dev.aetherstudioz.lang.kotlin.index.KotlinMainIndex
import dev.aetherstudioz.lang.kotlin.index.KotlinMembersIndex
import dev.aetherstudioz.lang.kotlin.index.KotlinPackageDeclIndex
import dev.aetherstudioz.lang.kotlin.index.KotlinPackageTypesIndex
import dev.aetherstudioz.lang.kotlin.index.KotlinPackagesIndex
...
import dev.aetherstudioz.lang.kotlin.index.KotlinSourceSymbolsIndex
...
import dev.aetherstudioz.lang.xml.lint.XmlAnalysisSupport
import dev.aetherstudioz.model.FacetCodecRegistry
import dev.aetherstudioz.model.FileIconRegistry
import dev.aetherstudioz.model.MODULE_SOURCES
...
import dev.aetherstudioz.model.sync.BUILD_FILE_WRITER_EP
import dev.aetherstudioz.model.sync.PROJECT_IMPORTER_EP
...
import dev.aetherstudioz.ui.ext.UiPlugin
 ...
 * companion is now a set of built-in [Plugin]s driven by the [dev.aetherstudioz.plugin.impl.PluginManager]. Each maps
 * 1:1 to the [dev.aetherstudioz.platform.PluginId] it contributed under before, so the resolved registry is identical.
 *
 ...
 * a [dev.aetherstudioz.model.Workspace] or [dev.aetherstudioz.model.Module] to resolve through, as the synthetic `R` does: that
 ...
 * ecj lives on as the build compiler ([dev.aetherstudioz.lang.jdt.build] / `:jvm-build`) and as the `.java` compile-level
 * [dev.aetherstudioz.analysis.DiagnosticProvider] + quick-fixes ([JdtAnalysisPlugin]). This plugin just registers the
 ...
 * it drops the only block mapping, so the engine's [dev.aetherstudioz.core.services.BlockService] reports no mappings and
...
private class KotlinSupportPlugin : Plugin {
    ...
    override fun register(reg: PluginRegistration) {
        ...
        reg.register(
            dev.aetherstudioz.lang.kotlin.symbols.KOTLIN_SYNTHETIC_MEMBER_EP,
            dev.aetherstudioz.lang.kotlin.symbols.SerializationSyntheticMembers,
        )
        reg.register(
            dev.aetherstudioz.lang.kotlin.symbols.KOTLIN_SYNTHETIC_MEMBER_EP,
            dev.aetherstudioz.lang.kotlin.symbols.ParcelizeSyntheticMembers,
        )
        reg.contributeVia { ext, pid ->
            val templates = ProjectTemplateRegistry(ext)
...
private class PluginDevPlugin(private val env: ApplicationEnvironment) : Plugin {
    override val manifest = PluginManifest(
        ...
        description = "Author AetherStudioZ plugins: the Create-Project template for a plugin packaged as its " +
            "own app, and editor checks for its packaged manifest.",
        dependsOn = listOf("android-support"),
    )
    override fun register(reg: PluginRegistration) {
        reg.contributeVia { ext, pid ->
            ProjectTemplateRegistry(ext).register(AetherStudioZPluginTemplate, pid)
...
private class JavaPsiAnalysisPlugin : Plugin {
    ...
    override fun register(reg: PluginRegistration) {
        reg.contributeVia { ext, pid -> dev.aetherstudioz.lang.java.analysis.JavaPsiAnalysisSupport.register(ext, pid) }
    }
}
