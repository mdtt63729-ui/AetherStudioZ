...
import dev.aetherstudioz.android.support.DefaultProguardFiles
...
/**
 * Writes a native AetherStudioZ project out as a real Gradle project: the model (`workspace.json` +
 ...
 * reverse direction is [dev.aetherstudioz.core.GradleImport]; the two are symmetric enough that an export re-imports.
 */
object GradleProjectExport {
    ...
    private fun readModules(
        ...
        if (compose && features?.compose != true) {
            notes += "Module '${data.name}' declares Compose libraries, so the export applies the Compose " +
                "compiler plugin. AetherStudioZ turns it on from the classpath instead of a build flag."
        }
        Exported(
            data = data,
            dir = dir,
        ...
        k.block("buildTypes") {
            for (type in interesting) {
                ...
                block("$creator(\"${escape(type.name)}\")") {
                    ...
                    if (signing != null) {
                        line("// Signed with the '${escape(signing)}' keystore in AetherStudioZ. Add a")
    ...
    private fun libraryDependency(
        ...
        if (parts.size !in 2..3 || parts.any { it.isBlank() }) {
            ...
            k.line("// $configuration(\"$name\") // no Maven coordinate; ${if (jars > 0) "$jars file(s) in AetherStudioZ" else "unresolved"}")
    ...
    private fun kspComment(k: Kts, module: Exported, notes: MutableList<String>) {
        ...
        k.line()
        k.line("// AetherStudioZ ran these annotation processors from its own bundle: $named.")
        ...
        notes += "Module '${module.data.name}' uses the bundled annotation processors ($named). AetherStudioZ " +
    ...
    private fun extraModuleFiles(module: Exported): Map<String, String> {
        ...
        for (type in facet.buildTypes) {
            ...
            val text = buildString {
                appendLine("# Inline R8 rules from the '${type.name}' build type, written out by AetherStudioZ.")
    ...
    private fun gradleProperties(modules: List<Exported>, android: Boolean): String = buildString {
        appendLine("# Generated by AetherStudioZ.")
    ...
    private fun notesDocument(project: ProjectData, modules: List<Exported>, notes: List<String>): String =
        buildString {
            ...
            appendLine()
            appendLine("Exported from AetherStudioZ as a Gradle project. The build files here were generated from")
            ...
            appendLine()
            appendLine("Kotlin is pinned to the version AetherStudioZ compiled this project with, so the sources")
    ...
    private class Kts(private val startDepth: Int = 0) {
        ...
        fun header() {
            line("// Generated by AetherStudioZ from the project model. Review before you rely on it.")
            line()
        }
