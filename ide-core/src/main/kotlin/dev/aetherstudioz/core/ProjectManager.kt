...
import dev.aetherstudioz.android.support.resources.LauncherIcon
import dev.aetherstudioz.android.support.tools.KeystoreRegistry
import dev.aetherstudioz.build.engine.ProgramInterpreter
import dev.aetherstudioz.core.gradle.GradleProjectExport
...
import dev.aetherstudioz.model.impl.ProjectData
...
import dev.aetherstudioz.platform.ServiceContainer
import dev.aetherstudioz.platform.ServiceKey
import dev.aetherstudioz.platform.impl.ApplicationContainer
import dev.aetherstudioz.platform.impl.PlatformCore
import dev.aetherstudioz.platform.settings.PreferenceReader
import dev.aetherstudioz.platform.settings.SETTINGS_ACCESS
import dev.aetherstudioz.platform.settings.SettingsAccess
...
data class ProjectSummary(
     ...
     *  rather than presenting it as a AetherStudioZ project it is not. */
...
enum class ImportableKind {
    /** A AetherStudioZ workspace (`.platform/workspace.json`) -- adopted verbatim, nothing to translate. */
    CODE_ASSIST,

    /** A foreign build system some [dev.aetherstudioz.model.sync.ProjectImporter] claims (Gradle today) -- read
...
class ProjectManager private constructor(
    ...
    private val customViewRuntime: dev.aetherstudioz.preview.impl.CustomViewRuntime? = null,
    /** On-device real-view layout renderer (from :ide-android): the layoutlib-on-device preview path. */
    private val realViewRuntime: dev.aetherstudioz.preview.impl.RealViewRuntime? = null,
    ...
    private val kotlinPluginLoader: dev.aetherstudioz.lang.kotlin.compile.KotlinPluginLoader? = null,
    ...
    private val kotlinCompiler: dev.aetherstudioz.lang.kotlin.compile.KotlinCompilerBackend? = null,
    ...
    private val pluginSources: List<dev.aetherstudioz.plugin.external.PluginSource> = emptyList(),
     ...
     * [dev.aetherstudioz.model.sync.ProjectImporter] that claims it, and open it. Returns null when no importer
    ...
    fun importExternalProject(sourceDir: Path): IdeServices? {
        // Already a AetherStudioZ workspace: adopt it verbatim. No importer claims such a folder -- there is no
     ...
     *
     * Returns false when nothing claims the folder — a download that is not a project AetherStudioZ can open.
    ...
    fun adoptProjectInPlace(dir: Path): Boolean {
        ...
        // Packaged by AetherStudioZ (the usual case, since submissions are zipped by the IDE): the model
    ...
    fun setPreference(key: String, value: String) {
        ...
        Files.newOutputStream(prefsFile).use { props.store(it, "AetherStudioZ preferences") }
    }
    ...
    fun exportBackup(): Path {
        ...
        val dest = exportsDir.resolve("aetherstudioz-backup-${System.currentTimeMillis()}.zip")
    ...
    companion object {
        ...
        fun onDevice(
            ...
            customViewRuntime: dev.aetherstudioz.preview.impl.CustomViewRuntime? = null,
            /** The host's real-view layout renderer (layoutlib-on-device), so the preview can render real views. */
            realViewRuntime: dev.aetherstudioz.preview.impl.RealViewRuntime? = null,
            ...
            kotlinPluginLoader: dev.aetherstudioz.lang.kotlin.compile.KotlinPluginLoader? = null,
            ...
            kotlinCompiler: dev.aetherstudioz.lang.kotlin.compile.KotlinCompilerBackend? = null,
            ...
            r8Shrinker: dev.aetherstudioz.android.support.tools.Shrinker? = null,
            /** The host's forked-VM D8 dexer for the dex merge step (debug-path memory peak). Null → in-process. */
            r8MergeDexer: dev.aetherstudioz.android.support.tools.Dexer? = null,
            /** Max class-dex per merge batch on a large app (the "Dex merge batch size" setting); read per build. */
            mergeChunkProvider: () -> Int = { dev.aetherstudioz.core.settings.BuiltInSettingsPages.DEX_MERGE_BATCH_DEFAULT },
            ...
            pluginSources: List<dev.aetherstudioz.plugin.external.PluginSource> = emptyList(),
            /** The running IDE's version, checked against an installed plugin's `minHostVersion`. */
