 ...
 *  [dev.aetherstudioz.store.StoreCatalogSource.Unconfigured], and the store falls back to the bundled catalog. */
val STORE_CATALOG_SOURCE = ServiceKey<dev.aetherstudioz.store.StoreCatalogSource>("platform.storeCatalog")
...
val STORE_ACCOUNT_SERVICE = ServiceKey<dev.aetherstudioz.store.StoreAccountService>("platform.storeAccounts")
...
val STORE_SUBMISSION_SERVICE = ServiceKey<dev.aetherstudioz.store.StoreSubmissionService>("platform.storeSubmissions")

/** Ratings and reviews. Reads are anonymous; writes need the account service, so the launcher builds both. */
val STORE_REVIEW_SERVICE = ServiceKey<dev.aetherstudioz.store.StoreReviewService>("platform.storeReviews")
...
interface NotificationPresenter {
    fun present(notification: dev.aetherstudioz.ui.backend.UiNotification)
}
