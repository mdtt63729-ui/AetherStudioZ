...
import dev.aetherstudioz.android.support.metadata.AttributeSpec
import dev.aetherstudioz.android.support.metadata.LayoutMetadata
import dev.aetherstudioz.android.support.metadata.Widget
import dev.aetherstudioz.android.support.resources.AndroidManifestCatalog
import dev.aetherstudioz.android.support.resources.AndroidToolsCatalog
    ...
    private fun valueItemsFor(spec: AttributeSpec, prefix: String): List<CompletionItem> {
        ...
        for (type in spec.resourceTypes) {
            ...
            for (c in runCatching { resources(type) }.getOrDefault(emptyList())) {
                ...
                // Show the resolved value as the hint (@string/app_name → "AetherStudioZ", @color/primary → #6200EE)
