package dev.aetherstudioz.core.templates
...
import dev.aetherstudioz.model.template.TemplateDependency
import dev.aetherstudioz.model.template.TemplateId
import dev.aetherstudioz.model.template.TemplateParameter
import dev.aetherstudioz.model.template.TextValidation
import dev.aetherstudioz.plugin.PLUGIN_API_VERSION
import dev.aetherstudioz.plugin.PLUGIN_SPI_VERSION

/**
 * A AetherStudioZ plugin, scaffolded as the app the IDE loads it from: a packaged TOML manifest under
 * `res/raw`, the marker activity the package manager is queried for, and an entry-point class implementing
 * `Plugin` (the engine facet) or `dev.aetherstudioz.plugin.ui.UiPlugin` (the Compose UI facet). Building it produces an
 ...
 * no declaration: AetherStudioZ applies it to any module whose classpath carries the Compose runtime.
 */
object AetherStudioZPluginTemplate : ProjectTemplate {

    override val id = TemplateId("aetherstudioz-plugin")
    override val displayName = "AetherStudioZ Plugin"
    ...
    override fun dependencies(args: TemplateArgs): List<TemplateDependency> = buildList {
        add(TemplateDependency(MODULE, "io.github.aetherstudioz:plugin-api:$PLUGIN_SPI_VERSION", scope = "compileOnly"))
        add(TemplateDependency(MODULE, "io.github.aetherstudioz:platform-core:$PLUGIN_SPI_VERSION", scope = "compileOnly"))
        if (args.string(CONTRIBUTES, BOTH) == PANEL) {
            add(
                TemplateDependency(
                    MODULE,
                    "io.github.aetherstudioz:plugin-ui-api:$PLUGIN_SPI_VERSION",
                    scope = "compileOnly",
                ),
            )
    ...
    override fun generate(scaffold: ProjectScaffold, args: TemplateArgs) {
        ...
        scaffold.writeText(
            "$MODULE/src/main/res/raw/aetherstudioz_plugin.toml",
    ...
    private fun manifestToml(
        ...
        return """
            ...
            description = "${args.name}, a AetherStudioZ plugin."
            $entryPoints
    ...
    private fun androidManifest(): String = """
        ...
        <manifest xmlns:android="http://schemas.android.com/apk/res/android">
            ...
            <application
                ...
                <!-- How AetherStudioZ finds this app at all: it queries the package manager for this action,
                ...
                <activity
                    ...
                    <meta-data
                        android:name="dev.aetherstudioz.aetherstudioz.plugin.manifest"
    ...
    private fun entryPoint(
        ...
        val imports = buildList {
            if (settings) {
                add("import dev.aetherstudioz.platform.settings.PreferenceReader")
                add("import dev.aetherstudioz.platform.settings.SETTINGS_PAGE_EP")
                add("import dev.aetherstudioz.platform.settings.SettingControl")
                add("import dev.aetherstudioz.platform.settings.SettingsPage")
            }
            add("import dev.aetherstudioz.plugin.Plugin")
            add("import dev.aetherstudioz.plugin.PluginRegistration")
            if (command) {
                add("import dev.aetherstudioz.plugin.action.ActionPlaces")
                add("import dev.aetherstudioz.plugin.action.ActionResult")
                add("import dev.aetherstudioz.plugin.action.SimpleAction")
                add("import dev.aetherstudioz.plugin.action.UI_ACTION_EP")
            }
        }.joinToString("\n")
...
/**
 * The entry point named by `res/raw/aetherstudioz_plugin.toml`. The IDE instantiates it off the installed APK
...
import dev.aetherstudioz.plugin.ui.ToolWindow
import dev.aetherstudioz.plugin.ui.ToolWindowAnchor
import dev.aetherstudioz.plugin.ui.UiContext
import dev.aetherstudioz.plugin.ui.UiPlugin
import dev.aetherstudioz.plugin.ui.UiRegistration

/**
 * The UI facet named by `res/raw/aetherstudioz_plugin.toml`. It runs inside the IDE's process and composes into
 ...
 * and not before.
 *
 * To add an engine facet (services, analyzers, commands, settings pages), implement `dev.aetherstudioz.plugin.Plugin`
...
"""
    ...
    private fun infoActivity(pkg: String, entryClass: String): String = """
         ...
         * AetherStudioZ plugin. The code that matters runs inside the IDE, not here.
         */
        class PluginInfoActivity : Activity() {
            ...
            override fun onCreate(savedInstanceState: Bundle?) {
                ...
                body.addView(
                    text(
                        "A AetherStudioZ plugin.\n\n" +
                            "Install this app, then open AetherStudioZ and look under " +
    ...
    private fun readme(name: String, pluginId: String, entryClass: String, panel: Boolean): String {
        ...
        `$entryClass` implements `dev.aetherstudioz.plugin.ui.UiPlugin`, so it contributes UI that renders itself: a
        tool window here, and `screen` / `overlay` from the same registration object.

        - Compose is `compileOnly` and pinned to the version AetherStudioZ bundles. Your `@Composable` code is
          compiled by the Compose compiler plugin (AetherStudioZ applies it automatically once the Compose
        ...
        - The panel gets a `UiContext`: where the user is, and how to open a file or another screen.
          Everything else belongs in an engine facet (`dev.aetherstudioz.plugin.Plugin`, in `entryPoints`), which has
        ...
        - Icons are ids in AetherStudioZ's own registry: a plugin has no `Context` for its own package, so it
          cannot ship drawables or use `stringResource`.
"""
        return """
        # $name

        A AetherStudioZ plugin, packaged as its own app. AetherStudioZ finds it through the package manager, reads
        `$MODULE/src/main/res/raw/aetherstudioz_plugin.toml`, and loads `$entryClass` off the installed APK.

        ## Try it

        Build and install this project, then restart AetherStudioZ and open **Settings > Plugins > Installed**.
        ...
        | Part | Where |
        | --- | --- |
        | The packaged manifest | `$MODULE/src/main/res/raw/aetherstudioz_plugin.toml` |
        | The marker activity AetherStudioZ queries for | `$MODULE/src/main/AndroidManifest.xml` |
    ...
    private val PROGUARD_RULES = """
        ...
        # res/raw/aetherstudioz_plugin.toml, and R8 would rename it.
        -keep class ** { *; }
    """.trimIndent() + "\n"
}
