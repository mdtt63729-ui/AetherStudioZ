...
import dev.aetherstudioz.ui.backend.NotificationService
import dev.aetherstudioz.ui.backend.UiNotification
import dev.aetherstudioz.ui.backend.UiNotificationKind
import dev.aetherstudioz.ui.backend.UiNotificationTarget
import java.io.File
...
internal class NotificationCenter(
    ...
    private fun load(): List<UiNotification> {
        ...
        val root = dev.aetherstudioz.platform.JsonReader.parseOrNull(text) ?: return emptyList()
        return dev.aetherstudioz.platform.JsonReader.arr(dev.aetherstudioz.platform.JsonReader.obj(root)?.get("notifications"))
            .mapNotNull { parse(it) }
            .take(MAX_KEPT)
    }

    private fun parse(value: Any?): UiNotification? {
        val id = dev.aetherstudioz.platform.JsonReader.str(value, "id") ?: return null
        val kindName = dev.aetherstudioz.platform.JsonReader.str(value, "kind")
        ...
        return UiNotification(
            id = id,
            kind = kind,
            title = dev.aetherstudioz.platform.JsonReader.str(value, "title") ?: return null,
            body = dev.aetherstudioz.platform.JsonReader.str(value, "body"),
            timestampMs = dev.aetherstudioz.platform.JsonReader.long(value, "timestampMs", 0L),
            read = dev.aetherstudioz.platform.JsonReader.bool(value, "read", false),
            target = parseTarget(value),
            key = dev.aetherstudioz.platform.JsonReader.str(value, "key"),
        )
    }

    private fun parseTarget(value: Any?): UiNotificationTarget? {
        val type = dev.aetherstudioz.platform.JsonReader.str(value, "targetType") ?: return null
        val id = dev.aetherstudioz.platform.JsonReader.str(value, "targetId")
        return when (type) {
