...
import dev.aetherstudioz.core.CaprojFormat
import dev.aetherstudioz.core.ProjectIconLocator
import dev.aetherstudioz.core.ProjectPackaging
...
import dev.aetherstudioz.platform.storage.StorageUsage
...
import dev.aetherstudioz.ui.backend.UiExportOptions
import dev.aetherstudioz.ui.backend.UiExportPlan
import dev.aetherstudioz.ui.backend.UiGradleExport
import dev.aetherstudioz.ui.backend.UiImportPreview
import dev.aetherstudioz.ui.backend.UiPackagedEntry
import dev.aetherstudioz.ui.backend.UiPackagedModule
import dev.aetherstudioz.ui.backend.UiProjectIcon
import dev.aetherstudioz.ui.backend.UiOpenTab
import dev.aetherstudioz.ui.backend.UiOpenTabs
import dev.aetherstudioz.ui.backend.UiConvertResult
import dev.aetherstudioz.ui.backend.UiProjectFolderKind
import dev.aetherstudioz.ui.backend.UiProjectResult
...
import dev.aetherstudioz.ui.backend.UiSyncResult
import dev.aetherstudioz.ui.backend.UiTemplateParam
import dev.aetherstudioz.ui.backend.UiUnrecognizedProject
        ...
        return withContext(Dispatchers.IO) {
            runCatching {
                val next = mgr.importExternalProject(Paths.get(sourceRootPath))
                    ?: return@runCatching UiProjectResult(false, "That folder isn't a AetherStudioZ or Gradle project.")
                ctx.swapEngine(next)
        ...
        return withContext(Dispatchers.IO) {
            runCatching {
                ...
                UiImportPreview(
                    ...
                    else "This package was created by a newer version of AetherStudioZ. Update to import it.",
            ...
            runCatching {
                val next = mgr.importProject(archivePath, projectName)
                    ?: return@runCatching UiProjectResult(
                        false,
                        "That file isn't a AetherStudioZ project package, or it needs a newer version of AetherStudioZ.",
                    )
                ctx.swapEngine(next)
                UiProjectResult(true, "Imported ${next.projectDisplayName()}", next.workspaceRoot.toString())
            }.getOrElse { e ->
