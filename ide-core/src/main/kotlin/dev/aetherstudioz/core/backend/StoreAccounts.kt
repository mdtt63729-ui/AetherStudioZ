...
internal class StoreAccounts(
    private val accounts: dev.aetherstudioz.store.StoreAccountService,
    /** Asked which providers the backend currently allows. Null keeps whatever the build supports. */
    private val source: dev.aetherstudioz.store.StoreCatalogSource? = null,
) {
    ...
    fun refreshProviders() {
        ...
        if (result is dev.aetherstudioz.store.StoreResult.Ok) allowed = result.value
    }
    ...
    fun beginSignIn(provider: String): String? {
        val wanted = dev.aetherstudioz.store.StoreProvider.entries.firstOrNull { it.wire == provider } ?: return null
    ...
    private fun dev.aetherstudioz.store.StoreAccount.toUi() = UiStoreAccount(
        userId = userId,
        email = email,
        handle = handle,
