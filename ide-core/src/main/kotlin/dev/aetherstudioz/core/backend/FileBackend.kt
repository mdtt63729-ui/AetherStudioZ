...
import dev.aetherstudioz.core.TreeRootInfo
...
internal class FileBackend(private val ctx: BackendContext) : FileService {
     ...
     * each level has exactly one subdirectory and no files, so `com/ → tyron/ → aetherstudioz/` becomes one
     * `com.aetherstudioz.aetherstudioz` node. The chain's [PackageSegment]s (one per level, each with its dotted
