...
import dev.aetherstudioz.ui.backend.UiStoreInstallResult
import dev.aetherstudioz.ui.backend.UiStoreItem
import dev.aetherstudioz.ui.backend.UiStoreItemKind
import dev.aetherstudioz.ui.backend.UiInstallProgress
import dev.aetherstudioz.ui.backend.UiInstallState
import dev.aetherstudioz.ui.backend.UiSignInPhase
import dev.aetherstudioz.ui.backend.UiStoreAccount
import dev.aetherstudioz.ui.backend.UiStoreAuthState
import dev.aetherstudioz.ui.backend.UiStoreFeed
import dev.aetherstudioz.ui.backend.UiStoreSection
...
internal class StoreBackend(
    ...
    private val source: dev.aetherstudioz.store.StoreCatalogSource = dev.aetherstudioz.store.StoreCatalogSource.Unconfigured,
    private val accounts: dev.aetherstudioz.store.StoreAccountService = dev.aetherstudioz.store.StoreAccountService.Unsupported,
    private val submissions: dev.aetherstudioz.store.StoreSubmissionService =
    ...
    private val reviewService: dev.aetherstudioz.store.StoreReviewService =
    ...
    override suspend fun feed(seedItemId: String?): UiStoreFeed? {
        ...
        return withContext(dev.aetherstudioz.core.backend.storeIo) {
            ...
            when (val result = source.feedDocument(seed)) {
                is dev.aetherstudioz.store.StoreResult.Ok -> {
                    val parsed = dev.aetherstudioz.store.impl.StoreFeedParser.parse(result.value)
                    if (parsed == null) {
    ...
    override suspend fun screenshotFile(storagePath: String): String? = withContext(storeIo) {
        ...
        when (source.downloadMedia(storagePath, cached)) {
            is dev.aetherstudioz.store.StoreResult.Ok -> cached.absolutePath
    ...
    override suspend fun publisherProfile(handle: String): dev.aetherstudioz.ui.backend.UiPublisherProfile? =
        withContext(storeIo) {
            when (val result = reviewService.publisherProfile(handle)) {
                is dev.aetherstudioz.store.StoreResult.Ok -> result.value?.let { p ->
                    dev.aetherstudioz.ui.backend.UiPublisherProfile(
                        handle = p.handle,
                ...
                is dev.aetherstudioz.store.StoreResult.Unavailable ->
                    dev.aetherstudioz.ui.backend.UiPublisherProfile(handle, handle, error = result.reason)
                is dev.aetherstudioz.store.StoreResult.Failed ->
                    dev.aetherstudioz.ui.backend.UiPublisherProfile(handle, handle, error = result.message)
            }
        }
    ...
    override suspend fun setFollowing(handle: String, following: Boolean): String? = withContext(storeIo) {
        when (val result = reviewService.setFollowing(handle, following)) {
            is dev.aetherstudioz.store.StoreResult.Ok -> null
            is dev.aetherstudioz.store.StoreResult.Unavailable -> result.reason
            is dev.aetherstudioz.store.StoreResult.Failed -> result.message
        }
    }
    ...
    override suspend fun reviews(
        itemId: String,
        sort: dev.aetherstudioz.ui.backend.UiReviewSort,
        limit: Int,
    ): dev.aetherstudioz.ui.backend.UiReviewPage = withContext(storeIo) { reviewState.page(itemId, sort, limit) }
    ...
    override suspend fun reportReview(
        itemId: String,
        authorId: String,
        reason: dev.aetherstudioz.ui.backend.UiReportReason,
        detail: String?,
    ...
    override suspend fun packProject(rootPath: String): dev.aetherstudioz.ui.backend.UiPackagedProject? =
    ...
    override suspend fun submitCategories(): List<Pair<String, String>> = withContext(storeIo) {
        when (val result = source.categories()) {
            is dev.aetherstudioz.store.StoreResult.Ok -> result.value
    ...
    override suspend fun submit(
        draft: dev.aetherstudioz.ui.backend.UiSubmissionDraft,
        packaged: dev.aetherstudioz.ui.backend.UiPackagedProject,
    ): dev.aetherstudioz.ui.backend.UiSubmitResult = withContext(storeIo) {
        ...
        result.submission?.let { sub ->
            notifications?.post(
                kind = dev.aetherstudioz.ui.backend.UiNotificationKind.STORE_SUBMISSION,
                title = "${draft.title} is in review",
                body = "A moderator reviews every submission. Nothing is public until it is approved.",
                target = dev.aetherstudioz.ui.backend.UiNotificationTarget.Submissions,
     ...
     */
    override suspend fun mySubmissions(): List<dev.aetherstudioz.ui.backend.UiStoreSubmission> = withContext(storeIo) {
    ...
    private fun submissionHeadline(sub: dev.aetherstudioz.ui.backend.UiStoreSubmission): String = when (sub.status) {
        dev.aetherstudioz.ui.backend.UiSubmissionStatus.PUBLISHED -> "${sub.projectName} is live in the store"
        dev.aetherstudioz.ui.backend.UiSubmissionStatus.REJECTED -> "${sub.projectName} was not accepted"
        dev.aetherstudioz.ui.backend.UiSubmissionStatus.CHANGES_REQUESTED -> "${sub.projectName} needs changes"
        dev.aetherstudioz.ui.backend.UiSubmissionStatus.BUILDING -> "${sub.projectName} is being built"
        dev.aetherstudioz.ui.backend.UiSubmissionStatus.SUBMITTED -> "${sub.projectName} is in review"
    }
    ...
    private fun cachedFeed(bundled: Map<String, UiStoreItem>): UiStoreFeed? {
        ...
        val parsed = dev.aetherstudioz.store.impl.StoreFeedParser.parse(raw) ?: return null
    ...
    override suspend fun install(id: String, args: Map<String, String>): UiStoreInstallResult {
        ...
        // picker; the UI only has to be told to ask again (AetherStudioZAppState.refreshProjects).
        return withContext(storeIo) {
            val result = installer.install(
                ...
                adopt = { dir ->
                    ...
                    if (ok) null else "That download isn't a project AetherStudioZ can open"
                },
    ...
    private fun rememberPayloads(feed: dev.aetherstudioz.store.StoreFeed) {
