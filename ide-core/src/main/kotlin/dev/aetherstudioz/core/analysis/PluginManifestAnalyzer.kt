package dev.aetherstudioz.core.analysis
...
import dev.aetherstudioz.plugin.PluginVersions
import java.nio.file.Files
import java.nio.file.Path
import java.nio.file.Paths

/**
 * Editor checks for a plugin's packaged manifest, `res/raw/aetherstudioz_plugin.toml`.
 *
...
class PluginManifestAnalyzer(
    ...
    override val id = AnalyzerId("aetherstudiozPluginManifest")
    override val displayName = "AetherStudioZ plugin manifest problems"
    ...
    override fun analyze(target: AnalysisTarget, sink: DiagnosticSink) {
        ...
        if (min != null && !PluginVersions.satisfies(host, min)) {
            sink.report(
                ...
                Severity.ERROR,
                "Requires AetherStudioZ $min or newer; this IDE is $host, so it would not load here.",
                CODE,
            )
        }
    ...
    private fun checkCapabilities(
        manifest: dev.aetherstudioz.plugin.PluginManifest,
        ranges: KeyRanges,
        sink: DiagnosticSink,
    ) {
    ...
    private companion object {
        const val PLUGIN_INTERFACE = "dev.aetherstudioz.plugin.Plugin"
        const val UI_PLUGIN_INTERFACE = "dev.aetherstudioz.plugin.ui.UiPlugin"
        const val CODE = "aetherstudiozPlugin"
    }
}
