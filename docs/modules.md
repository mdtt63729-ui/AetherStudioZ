...
|---|---|---|
| `platform-core` | `dev.aetherstudioz.platform` | Extension registry, message bus, model read/write lock, activities/progress, disposer, content hashing, plugin ids. |
| `vfs-api` | `dev.aetherstudioz.vfs` | `VirtualFile`, `VirtualFileSystem`, file events, listeners. |
| `project-model-api` | `dev.aetherstudioz.model`, `.graph`, `.sync` | Workspace/Project/Module/SourceSet/ContentRoot, order entries + scopes (all open, so a plugin's language can name its own roles/platform/packaging/level/scopes), classpath snapshots, library/SDK tables, module types, variants, facets + their codecs, the registries that resolve all of those, transactions, the project/module graphs, file-icon SPI, the foreign-build-system sync SPI (`ProjectImporter` → `ExternalProjectModel`, `BuildFileWriter`), and `ModuleSources` + `MODULE_SOURCES`, the source-set/source-root slice of the engine's module service a plugin can resolve. |
| `build-api` | `dev.aetherstudioz.build` | `BuildSystem` SPI; the generic incremental task engine contracts (`Task`/`TaskInputs`/`TaskOutputs`/`TaskGraph`/`TaskExecutor`); the contribution seams (`BuildPlugin`, `RunTaskProvider`/`RunAction`); `BuildControl` + `BUILD_CONTROL`, the narrowed slice of the engine's build service a plugin can resolve. |
| `language-api` | `dev.aetherstudioz.lang`, `.dom`, `.incremental`, `.resolve`, `.completion` | `LanguageBackend` SPI, source analyzer/compiler contracts, the backend-neutral DOM, incremental parsing, symbol/scope/type resolution, code completion, and a module's analysis inputs (`CompilationContext`, the model binding that builds one, and the `CompilationContextProvider` a plugin supplies its own with). |
| `deps-api` | `dev.aetherstudioz.deps` | Dependency resolution SPI (Maven coordinates → jars/aars, conflict policy). |
| `vcs-api` | `dev.aetherstudioz.vcs` | Version-control SPI: the repository/branch/commit/status model, the `VcsProvider` extension point, and the account/credential/forge ports sign-in is built on. |
| `index-api` | `dev.aetherstudioz.index` | Indexing SPI: index extensions, index service, shared value types; `SymbolSearch` + `SYMBOL_SEARCH`, the resolvable symbol/member lookup over them. |
| `analysis-api` | `dev.aetherstudioz.analysis` | Diagnostics/analyzer/quick-fix SPI: one diagnostic model and one pipeline; compiler errors and analyzer findings merge into the same stream; `ModuleAnalysis` + `MODULE_ANALYSIS`, a module's per-language `SourceAnalyzer` as a resolvable service. |
| `block-api` | `dev.aetherstudioz.block` | Projectional (block) editor SPI: block tree, block mappings, block edits, the projection service. |
| `plugin-api` | `dev.aetherstudioz.plugin`, `.action`, `.external` | The plugin SPI (`Plugin`/`PluginManifest`/`PluginRegistration`), the lean action model (`IdeAction`/`ActionGroup` + named places), and the discovery SPI for plugins the user installed separately (`PluginSource`/`DiscoveredPlugin`/`PluginOrigin`). |
| `plugin-ui-api` | `dev.aetherstudioz.plugin.ui` | The UI half of the plugin SPI, for a plugin shipped as its own app: the `UiPlugin` facet named by the manifest's `uiEntryPoints`, its tool window / screen / overlay contributions, and the narrow `UiContext` their `@Composable` bodies render against. Compose is `compileOnly` (the host provides it) and it depends on no other AetherStudioZ module, so publishing it commits to a surface `IdeBackend` changes cannot break. |
