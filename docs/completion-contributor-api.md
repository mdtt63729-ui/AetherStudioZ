...
`CompletionEngine` (`ide-core`, `dev.aetherstudioz.core.completion`). The engine:
...
```kotlin
import dev.aetherstudioz.lang.completion.*
...
Target a position with an `ElementPattern<DomNode>` over the neutral DOM (`dev.aetherstudioz.lang.patterns`), so one
...
See `dev.aetherstudioz.lang.completion.Contributor`, `dev.aetherstudioz.lang.patterns.Patterns`,
`dev.aetherstudioz.lang.postfix.Postfix`, and `dev.aetherstudioz.core.completion.CompletionEngine`.
