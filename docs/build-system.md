...
`GradleImport` (with the `dev.aetherstudioz.core.gradle` reader primitives) is the tolerant, **non-evaluating** reader.
...
Studio or on a CI machine. `GradleProjectExport` (`ide-core`, `dev.aetherstudioz.core.gradle`) renders the model
