...
```kotlin
dependencies {
    compileOnly(platform("io.github.aetherstudioz:plugin-bom:2.0.0"))   // was 1.3.0
...
```toml
# res/raw/aetherstudioz_plugin.toml
apiVersion = 3   # was 2
```
...
`FileIconRegistry` and `FacetData` moved from `dev.aetherstudioz.model.impl` (in the unpublished
`:project-model-impl`) to `dev.aetherstudioz.model` in `project-model-api`. A plugin could not reach them before,
so this only adds surface. Update the import if you had a copy of them:

```kotlin
import dev.aetherstudioz.model.FacetCodecRegistry     // was dev.aetherstudioz.model.impl.FacetCodecRegistry
```
