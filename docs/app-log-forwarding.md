...
- **`IdeLogBridge`** — resolves the IDE's exported sink service by the `dev.aetherstudioz.applog.SINK` intent
...
1. a `<provider android:name=… android:authorities="<applicationId>.dev.aetherstudioz.applog">` (boots the bridge), and
2. a `<queries><intent><action android:name="dev.aetherstudioz.applog.SINK"/></intent></queries>` so the bridge can SEE
