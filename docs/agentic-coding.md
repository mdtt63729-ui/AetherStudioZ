...
The agent is AetherStudioZ's own agent. It is grounded as this IDE (on-device, interpreter-based runs,
...
```
agent-api   (dev.aetherstudioz.agent)        no engine deps; coroutines only
  ^
agent-impl  (dev.aetherstudioz.agent.impl)   OkHttp + okhttp-sse + kotlinx-serialization-json + platform-core
  ^
ide-core    (dev.aetherstudioz.core)         implements AgentWorkspace, hosts AgentBackend + AgentPlugin
ide-ui-api  (dev.aetherstudioz.ui.backend)   adds AgentService port + Ui* DTOs
ide-ui      (dev.aetherstudioz.ui)           the ChatDrawer + composables
```
...
- `SystemPrompt`: assembles the AetherStudioZ grounding plus live project context.
...
`AntigravityProvider` reaches Google's AetherStudioZ backend
...
With no project id, the free-tier project is discovered via `loadAetherStudioZ` (falling back to `onboardUser`).
...
`SystemPrompt` grounds the agent as AetherStudioZ and states the platform's real shape and limits: an
