# Writing AetherStudioZ plugins

AetherStudioZ is built as a plugin platform, and the IDE's own features are the first consumers of that
...
| Requirement | Why |
| --- | --- |
| The AetherStudioZ repository, buildable locally | Plugins are Gradle modules in the same build |
...
| Term | Meaning |
| --- | --- |
| **Extension point (EP)** | A typed, string-id-keyed slot that many implementations can be contributed to. `dev.aetherstudioz.platform.ExtensionPoint<T>` |
| **Extension** | One implementation contributed to an EP |
| **Extension registry** | The store of contributions, hierarchical (project registry parents the application registry). `dev.aetherstudioz.platform.ExtensionRegistry` |
| **Scoped service** | A lazily built, cached object bound to an APPLICATION / WORKSPACE / MODULE scope |
| **Engine plugin** | A `dev.aetherstudioz.plugin.Plugin`: data-driven contributions, no Compose |
| **UI plugin** | A `dev.aetherstudioz.ui.ext.UiPlugin`: Compose-bearing contributions |
| **Plugin id** | The stable string that attributes every contribution a plugin makes, e.g. `vcs` |
| **Manifest** | A plugin's identity and load-order metadata: `dev.aetherstudioz.plugin.PluginManifest` |
...
AetherStudioZ inverts it. [`ApplicationEnvironment`](../ide-core/src/main/kotlin/dev/ide/core/ApplicationEnvironment.kt)
...
```
                      ┌──────────────────────────────────────┐
   BuiltInPlugin  ───▶│ engine: dev.aetherstudioz.plugin.Plugin        │──▶ ExtensionRegistry (data-driven)
   (one feature)      │   manifest / register(reg)           │    services, EPs, settings, actions
                      ├──────────────────────────────────────┤
                      │ ui: dev.aetherstudioz.ui.ext.UiPlugin?         │──▶ UiPluginHost (Compose-bearing)
...
instantiates it once. Its UI facet implements a different, narrower interface (`dev.aetherstudioz.plugin.ui.UiPlugin`),
...
Package: `dev.aetherstudioz.plugin` in module [`:plugin-api`](../plugin-api).

### 3.1 `Plugin`

[`dev.aetherstudioz.plugin.Plugin`](../plugin-api/src/main/kotlin/dev/ide/plugin/Plugin.kt)
...
defaulted, because such a plugin's identity is its packaged `aetherstudioz_plugin.toml`, which the IDE reads
...
[`dev.aetherstudioz.plugin.PluginManifest`](../plugin-api/src/main/kotlin/dev/ide/plugin/PluginManifest.kt)
...
`manifest.pluginId` derives the `dev.aetherstudioz.platform.PluginId` used for attribution, so you never construct one
by hand.
...
[`dev.aetherstudioz.plugin.PluginRegistration`](../plugin-api/src/main/kotlin/dev/ide/plugin/PluginRegistration.kt)
...
[`dev.aetherstudioz.plugin.impl.PluginManager`](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/PluginManager.kt)
...
[`dev.aetherstudioz.plugin.impl.PluginCatalog`](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/PluginCatalog.kt) is
...
```kotlin
package dev.aetherstudioz.hello
...
| --- | --- |
| `dev.aetherstudioz.lang.LANGUAGE_BACKEND_EP` | A new editor language (parse, complete, navigate) |
| `dev.aetherstudioz.lang.COMPILATION_CONTEXT_PROVIDER_EP` | `platform.compilationContext` | `CompilationContextProvider` | The analysis inputs for your language |
| `dev.aetherstudioz.lang.FILE_TYPE_EP` | A file suffix → language mapping |
| `dev.aetherstudioz.analysis.ANALYZER_EP` | An inspection producing diagnostics |
| `dev.aetherstudioz.analysis.QUICK_FIX_PROVIDER_EP` | A fix for a diagnostic |
| `dev.aetherstudioz.analysis.ACTION_PROVIDER_EP` | A caret intention (no diagnostic needed) |
| `dev.aetherstudioz.index.INDEX_EP` | A persisted symbol index |
| `dev.aetherstudioz.build.BUILD_SYSTEM_EP` | Support for a new build system |
| `dev.aetherstudioz.build.BUILD_PLUGIN_EP` | Extra tasks wired into every build graph |
| `dev.aetherstudioz.build.SOURCE_GENERATOR_EP` | A code generator that runs before compilation |
| `dev.aetherstudioz.vcs.VCS_PROVIDER_EP` | Another version-control system |
| `dev.aetherstudioz.platform.settings.SETTINGS_PAGE_EP` | A settings category |
| `dev.aetherstudioz.plugin.action.UI_ACTION_EP` | A toolbar / menu / palette command |
...
`dev.aetherstudioz.model`) are the read side of these four EPs, if you need to resolve rather than contribute. They
...
[`dev.aetherstudioz.core.event.IdeEventTopics`](../ide-core/src/main/kotlin/dev/ide/core/event/IdeEventTopics.kt)
...
Lower-level spines are also on the same bus and available to you: `dev.aetherstudioz.vfs.VfsTopics` (raw file changes),
`dev.aetherstudioz.model.event.ProjectModelTopics` (model commits), and `dev.aetherstudioz.core.settings.SettingsTopics`.
...
[`dev.aetherstudioz.platform.settings.SettingsPage`](../platform-core/src/main/kotlin/dev/ide/platform/settings/Settings.kt)
...
AetherStudioZ has a hybrid action model. Choosing the right half is the main decision:
...
mirror is `dev.aetherstudioz.ui.backend.UiActionPlaces`.
...
Module: [`:ide-ui-api`](../ide-ui-api), package `dev.aetherstudioz.ui.ext`. This is the Compose-bearing half of the
...
```
BuiltInPlugins        ApplicationEnvironment          IdeBackend            AetherStudioZApp
...
kotlin {
    ...
    android {
        namespace = "dev.aetherstudioz.hello.ui"
        compileSdk = 36
        minSdk = 24
...
- **Keep your strings in your own module.** `compose.resources { publicResClass = false; packageOfResClass =
  "dev.aetherstudioz.hello.ui.generated.resources" }` keeps the generated accessor a private detail. See
...
Everything above is the **built-in** UI model (`dev.aetherstudioz.ui.ext.UiPlugin`), which lives in `ide-ui-api`.
A plugin shipped as its own app uses a second, narrower interface instead: `dev.aetherstudioz.plugin.ui.UiPlugin`, from
...
You can also write one inside AetherStudioZ itself. **New project > Plugin > AetherStudioZ Plugin** scaffolds
...
declaration, since AetherStudioZ applies it to any module whose classpath carries the Compose runtime. While
editing `aetherstudioz_plugin.toml` the IDE runs the same checks the loader makes (a malformed id, an
...
**1. The plugin manifest**, as `res/raw/aetherstudioz_plugin.toml`. This is `PluginManifest` in TOML, and it is
...
<activity android:name=".PluginInfoActivity" android:exported="true">
    <intent-filter>
        <action android:name="dev.aetherstudioz.aetherstudioz.action.PLUGIN" />
        <category android:name="android.intent.category.DEFAULT" />
    </intent-filter>
    <meta-data android:name="dev.aetherstudioz.aetherstudioz.plugin.manifest"
               android:resource="@raw/aetherstudioz_plugin" />
</activity>
```

**4. A UI facet**, if the plugin has UI. Implement `dev.aetherstudioz.plugin.ui.UiPlugin` from `plugin-ui-api` and name
it in `uiEntryPoints`:
...
dependencies {
    ...
    compileOnly(platform("io.github.aetherstudioz:plugin-bom:2.0.0"))

    compileOnly("io.github.aetherstudioz:plugin-ui-api")
...
```kotlin
compileOnly(platform("io.github.aetherstudioz:plugin-bom:2.0.0")) // one version for everything below

compileOnly("io.github.aetherstudioz:plugin-api")        // actions, menus, palette commands
compileOnly("io.github.aetherstudioz:platform-core")     // scoped services, settings pages, logging
compileOnly("io.github.aetherstudioz:project-model-api") // module types, templates, facets + codecs, file icons
compileOnly("io.github.aetherstudioz:language-api")      // file types, completion, postfix, compilation contexts
compileOnly("io.github.aetherstudioz:analysis-api")      // analyzers, diagnostics, quick fixes, intentions
compileOnly("io.github.aetherstudioz:index-api")         // persisted indexes
compileOnly("io.github.aetherstudioz:build-api")         // build systems, build plugins, tasks, source generators
compileOnly("io.github.aetherstudioz:plugin-ui-api")     // tool windows, screens, overlays (see part 4)
```

Take only the ones you use; each brings the ones below it transitively (`plugin-ui-api` brings nothing: it
depends on no other AetherStudioZ module).
...
`dev.aetherstudioz.plugin.ui.UiPlugin`, or both, and they do **not** declare a `PluginManifest`: the TOML above is this
...
dependencies {
    compileOnly(platform("io.github.aetherstudioz:plugin-bom:2.0.0"))
    compileOnly("io.github.aetherstudioz:plugin-api")
    compileOnly("io.github.aetherstudioz:platform-core")
}
```
...
whatever license you choose; the rest of AetherStudioZ is plain GPL-3.0-or-later. The SPI version is independent
...
| --- | --- | --- | --- |
| `dev.aetherstudioz.platform.SERVICE_EP` | `platform.service` | `ServiceDescriptor<*>` | A scoped service (use `reg.service(...)`) |
| `dev.aetherstudioz.platform.settings.SETTINGS_PAGE_EP` | `platform.settingsPage` | `SettingsPage` | A Settings category |

### Project model

| FQN | Id | Type | Contribute to add |
| --- | --- | --- | --- |
| `dev.aetherstudioz.model.ModuleTypeExtensionPoint` | `platform.moduleType` | `ModuleType` | A module type |
| `dev.aetherstudioz.model.FileIconExtensionPoint` | `platform.fileIcon` | `FileIconProvider` | File-tree icon classification |
| `dev.aetherstudioz.model.template.ProjectTemplateExtensionPoint` | `platform.projectTemplate` | `ProjectTemplate` | A Create-Project template |
| `dev.aetherstudioz.model.FACET_CODEC_EP` | `platform.facetCodec` | `FacetCodec<*>` | Persistence for a module facet |
| `dev.aetherstudioz.model.sync.PROJECT_IMPORTER_EP` | `platform.projectImporter` | `ProjectImporter` | Import of a foreign project layout |
| `dev.aetherstudioz.model.sync.BUILD_FILE_WRITER_EP` | `platform.buildFileWriter` | `BuildFileWriter` | Writing changes back to a build file |

### Languages, completion, analysis

| FQN | Id | Type | Contribute to add |
| --- | --- | --- | --- |
| `dev.aetherstudioz.lang.LANGUAGE_BACKEND_EP` | `platform.languageBackend` | `LanguageBackend` | A language backend |
| `dev.aetherstudioz.lang.FILE_TYPE_EP` | `platform.fileType` | `FileTypeMapping` | Suffix → language routing |
| `dev.aetherstudioz.lang.completion.COMPLETION_CONTRIBUTOR_EP` | `platform.completionContributor` | `CompletionContribution` | Completion items |
| `dev.aetherstudioz.lang.completion.COMPLETION_WEIGHER_EP` | `platform.completionWeigher` | `CompletionWeigher` | Completion ranking |
| `dev.aetherstudioz.lang.postfix.POSTFIX_TEMPLATE_EP` | `platform.postfixTemplate` | `PostfixTemplate` | A postfix template |
| `dev.aetherstudioz.lang.synthetic.SYNTHETIC_CLASS_EP` | `platform.syntheticClass` | `SyntheticClassProvider` | Generated classes the editor must see (e.g. `R`) |
| `dev.aetherstudioz.index.INDEX_EP` | `platform.index` | `IndexExtension<*, *>` | A persisted index |
| `dev.aetherstudioz.analysis.ANALYZER_EP` | `platform.analyzer` | `Analyzer` | An inspection |
| `dev.aetherstudioz.analysis.DIAGNOSTIC_PROVIDER_EP` | `platform.diagnosticProvider` | `DiagnosticProvider` | A diagnostic source |
| `dev.aetherstudioz.analysis.QUICK_FIX_PROVIDER_EP` | `platform.quickFixProvider` | `QuickFixProvider` | A fix for a diagnostic |
| `dev.aetherstudioz.analysis.ACTION_PROVIDER_EP` | `platform.actionProvider` | `ActionProvider` | A caret intention |
| `dev.aetherstudioz.block.BLOCK_MAPPING_EP` | `platform.blockMapping` | `BlockMapping` | Block-editor projection for a language |
| `dev.aetherstudioz.lang.kotlin.compile.KOTLIN_COMPILER_PLUGIN_EP` | `platform.kotlinCompilerPlugin` | `KotlinCompilerPlugin` | A Kotlin compiler plugin |
| `dev.aetherstudioz.lang.kotlin.symbols.KOTLIN_SYNTHETIC_MEMBER_EP` | `platform.kotlinSyntheticMember` | `KotlinSyntheticMemberProvider` | Editor visibility of compiler-generated members |
...
| FQN | Id | Type | Contribute to add |
| --- | --- | --- | --- |
| `dev.aetherstudioz.build.BUILD_SYSTEM_EP` | `platform.buildSystem` | `BuildSystem` | A build system |
| `dev.aetherstudioz.build.BUILD_PLUGIN_EP` | `platform.buildPlugin` | `BuildPlugin` | Tasks applied to every build graph |
| `dev.aetherstudioz.build.RUN_TASK_PROVIDER_EP` | `platform.runTaskProvider` | `RunTaskProvider` | Entries in the Run picker |
| `dev.aetherstudioz.build.SOURCE_GENERATOR_EP` | `platform.sourceGenerator` | `SourceGenerator` | Generated sources before compilation |

### Actions, VCS, Android

| FQN | Id | Type | Contribute to add |
| --- | --- | --- | --- |
| `dev.aetherstudioz.plugin.action.UI_ACTION_EP` | `platform.uiAction` | `IdeAction` | A toolbar / menu / palette command |
| `dev.aetherstudioz.plugin.action.ACTION_GROUP_EP` | `platform.actionGroup` | `ActionGroup` | Menu nesting |
| `dev.aetherstudioz.vcs.VCS_PROVIDER_EP` | `platform.vcsProvider` | `VcsProvider` | Another version-control system |
| `dev.aetherstudioz.android.support.icons.ICON_REPOSITORY_EP` | `platform.iconRepository` | `IconRepository` | A source for the Icon Manager |
...
| FQN | File |
| --- | --- |
| `dev.aetherstudioz.plugin.Plugin` | [Plugin.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/Plugin.kt) |
| `dev.aetherstudioz.plugin.PluginManifest` | [PluginManifest.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/PluginManifest.kt) |
| `dev.aetherstudioz.plugin.PluginRegistration` | [PluginRegistration.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/PluginRegistration.kt) |
| `dev.aetherstudioz.plugin.action.IdeAction` | [IdeAction.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/IdeAction.kt) |
| `dev.aetherstudioz.plugin.action.ActionGroup` | [ActionGroup.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionGroup.kt) |
| `dev.aetherstudioz.plugin.action.ActionPlace` / `ActionPlaces` | [ActionPlace.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionPlace.kt), [ActionPlaces.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionPlaces.kt) |
| `dev.aetherstudioz.plugin.action.ActionContext` | [ActionContext.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionContext.kt) |
| `dev.aetherstudioz.plugin.action.ActionResult` / `ActionEffect` | [ActionResult.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionResult.kt), [ActionEffect.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/ActionEffect.kt) |
| `dev.aetherstudioz.plugin.action.SimpleAction` / `SimpleGroup` | [Builders.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/Builders.kt) |
| `dev.aetherstudioz.plugin.action.UI_ACTION_EP` / `ACTION_GROUP_EP` | [Actions.kt](../plugin-api/src/main/kotlin/dev/ide/plugin/action/Actions.kt) |

### Engine runtime: [`:plugin-impl`](../plugin-impl)

| FQN | File |
| --- | --- |
| `dev.aetherstudioz.plugin.impl.PluginManager` | [PluginManager.kt](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/PluginManager.kt) |
| `dev.aetherstudioz.plugin.impl.PluginCatalog` | [PluginCatalog.kt](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/PluginCatalog.kt) |
| `dev.aetherstudioz.plugin.impl.PluginRegistrationImpl` | [PluginRegistrationImpl.kt](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/PluginRegistrationImpl.kt) |
| `dev.aetherstudioz.plugin.impl.ActionManager` | [ActionManager.kt](../plugin-impl/src/main/kotlin/dev/ide/plugin/impl/ActionManager.kt) |

### Substrate: [`:platform-core`](../platform-core)

| FQN | File |
| --- | --- |
| `dev.aetherstudioz.platform.ExtensionPoint` / `ExtensionRegistry` | [Platform.kt](../platform-core/src/main/kotlin/dev/ide/platform/Platform.kt) |
| `dev.aetherstudioz.platform.PluginId` / `Disposable` | [Platform.kt](../platform-core/src/main/kotlin/dev/ide/platform/Platform.kt) |
| `dev.aetherstudioz.platform.MessageBus` / `MessageBusConnection` / `Topic` | [Platform.kt](../platform-core/src/main/kotlin/dev/ide/platform/Platform.kt) |
| `dev.aetherstudioz.platform.ServiceKey` / `ServiceScope` / `ServiceContainer` / `ServiceScopeLevel` | [Services.kt](../platform-core/src/main/kotlin/dev/ide/platform/Services.kt) |
| `dev.aetherstudioz.platform.settings.SettingsPage` / `SettingControl` | [Settings.kt](../platform-core/src/main/kotlin/dev/ide/platform/settings/Settings.kt) |
| `dev.aetherstudioz.platform.log.Logger` / `Log` | [Log.kt](../platform-core/src/main/kotlin/dev/ide/platform/log/Log.kt) |

### Project model: [`:project-model-api`](../project-model-api)

| FQN | File |
| --- | --- |
| `dev.aetherstudioz.model.ModuleType` / `SourceSetTemplate` / `FacetTemplate` / `ContentRole` | [ProjectModel.kt](../project-model-api/src/main/kotlin/dev/ide/model/ProjectModel.kt) |
| `dev.aetherstudioz.model.PlatformKind` / `LanguageLevel` / `LibraryKind` / `DependencyScope` | [ProjectModel.kt](../project-model-api/src/main/kotlin/dev/ide/model/ProjectModel.kt) |
| `dev.aetherstudioz.model.Facet` / `FacetKey` / `FacetContainer` | [ProjectModel.kt](../project-model-api/src/main/kotlin/dev/ide/model/ProjectModel.kt) |
| `dev.aetherstudioz.model.FacetCodec` / `FacetData` / `FacetCodecRegistry` | [FacetCodec.kt](../project-model-api/src/main/kotlin/dev/ide/model/FacetCodec.kt) |
| `dev.aetherstudioz.model.ModuleTypeRegistry` / `UnknownModuleType` | [ModuleTypeRegistry.kt](../project-model-api/src/main/kotlin/dev/ide/model/ModuleTypeRegistry.kt) |
| `dev.aetherstudioz.model.ProjectTemplateRegistry` | [ProjectTemplateRegistry.kt](../project-model-api/src/main/kotlin/dev/ide/model/ProjectTemplateRegistry.kt) |
| `dev.aetherstudioz.model.FileIconRegistry` / `FileIconProvider` / `IconTarget` | [FileIconRegistry.kt](../project-model-api/src/main/kotlin/dev/ide/model/FileIconRegistry.kt) |
| `dev.aetherstudioz.model.template.ProjectTemplate` / `ProjectScaffold` / `TemplateParameter` | [ProjectTemplate.kt](../project-model-api/src/main/kotlin/dev/ide/model/template/ProjectTemplate.kt) |
| `dev.aetherstudioz.model.sync.ProjectImporter` / `ExternalProjectModel` / `BuildFileWriter` | [ProjectSync.kt](../project-model-api/src/main/kotlin/dev/ide/model/sync/ProjectSync.kt) |
| `dev.aetherstudioz.model.ModuleSources` | [ModuleSources.kt](../project-model-api/src/main/kotlin/dev/ide/model/ModuleSources.kt) |
...
| Type | What it is |
| --- | --- |
| `dev.aetherstudioz.plugin.ui.UiPlugin` | The UI facet an installed plugin implements; named by `uiEntryPoints` |
...
| FQN | File |
| --- | --- |
| `dev.aetherstudioz.ui.ext.UiPlugin` / `UiContributionScope` / `UiPluginHost` | [UiPlugin.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/UiPlugin.kt) |
| `dev.aetherstudioz.ui.ext.ToolWindowContribution` / `ToolWindowAnchor` / `ToolWindowContext` | [ToolWindows.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/ToolWindows.kt) |
| `dev.aetherstudioz.ui.ext.ScreenContribution` / `ScreenContext` / `ScreenRegistry` | [ToolWindows.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/ToolWindows.kt) |
| `dev.aetherstudioz.ui.ext.OverlayContribution` / `OverlayContext` | [ToolWindows.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/ToolWindows.kt) |
| `dev.aetherstudioz.ui.ext.EditorViewModeContribution` / `ViewModeContext` | [ToolWindows.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/ToolWindows.kt) |
| `dev.aetherstudioz.ui.ext.UiHostAction` / `SimpleUiAction` / `UiActionHost` / `UiDestinations` / `Registration` | [UiActions.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/UiActions.kt) |
| `dev.aetherstudioz.ui.ext.BuiltInUiPlugin` | [BuiltInUiActions.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/ext/BuiltInUiActions.kt) |
| `dev.aetherstudioz.ui.backend.IdeBackend` | [IdeBackend.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/backend/IdeBackend.kt) |
| `dev.aetherstudioz.ui.backend.UiActionPlaces` and the `Ui*` action DTOs | [IdeBackend.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/backend/IdeBackend.kt) |
| `dev.aetherstudioz.ui.icons.actionIcon` | [ActionIcons.kt](../ide-ui-api/src/commonMain/kotlin/dev/ide/ui/icons/ActionIcons.kt) |

### Host wiring: [`:ide-core`](../ide-core)

| FQN | File |
| --- | --- |
| `dev.aetherstudioz.core.ApplicationEnvironment` | [ApplicationEnvironment.kt](../ide-core/src/main/kotlin/dev/ide/core/ApplicationEnvironment.kt) |
| `dev.aetherstudioz.core.BuiltInPlugin` / `BuiltInPlugins` | [BuiltInPlugins.kt](../ide-core/src/main/kotlin/dev/ide/core/BuiltInPlugins.kt) |
| `dev.aetherstudioz.core.VcsPlugin` / `VcsSettingsPage` | [VcsPlugin.kt](../ide-core/src/main/kotlin/dev/ide/core/VcsPlugin.kt) |
| `dev.aetherstudioz.core.AgentPlugin` | [AgentPlugin.kt](../ide-core/src/main/kotlin/dev/ide/core/AgentPlugin.kt) |
| `dev.aetherstudioz.core.event.IdeEventTopics` | [IdeEventTopics.kt](../ide-core/src/main/kotlin/dev/ide/core/event/IdeEventTopics.kt) |
| `dev.aetherstudioz.core.IdeServicesBackend` | [IdeServicesBackend.kt](../ide-core/src/main/kotlin/dev/ide/core/IdeServicesBackend.kt) |
| `dev.aetherstudioz.core.ANALYTICS_SERVICE` and the platform-port `ServiceKey`s | [PlatformPorts.kt](../ide-core/src/main/kotlin/dev/ide/core/PlatformPorts.kt) |
...
| --- | --- | --- | --- |
| `dev.aetherstudioz.model.WORKSPACE_SERVICE` | WORKSPACE | `Workspace` | The bound workspace. The `ServiceScope.workspace()` helper is this lookup |
| `dev.aetherstudioz.build.BUILD_CONTROL` | WORKSPACE | `BuildControl` | Start or stop the build; compile and run a module's `main` and capture its output |
| `dev.aetherstudioz.index.SYMBOL_SEARCH` | WORKSPACE | `SymbolSearch` | Symbol and member lookup over the workspace indexes |
| `dev.aetherstudioz.model.MODULE_SOURCES` | WORKSPACE | `ModuleSources` | A module's source sets and source roots, including adding one |
| `dev.aetherstudioz.analysis.MODULE_ANALYSIS` | MODULE | `ModuleAnalysis` | The module's `SourceAnalyzer` per language: resolution and diagnostics for code the plugin did not parse |
