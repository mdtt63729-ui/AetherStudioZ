...
Three types carry the tier (`plugin-api`, `dev.aetherstudioz.plugin.external`):
...
Compose UI facets (`dev.aetherstudioz.plugin.ui.UiPlugin`, from the published `plugin-ui-api`). Either list alone is a
...
template (`AetherStudioZPluginTemplate`), the manifest checks (`PluginManifestAnalyzer`) and the manifest
completion (`PluginManifestCompletion`). A module counts as a plugin when it packages a
`res/raw/aetherstudioz_plugin.toml`, which `PluginProject` decides: the packaged manifest is what discovery
...
`io.github.aetherstudioz:plugin-api` / `:platform-core` coordinates at `PLUGIN_SPI_VERSION`, declared
