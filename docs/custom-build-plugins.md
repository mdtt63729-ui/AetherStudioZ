...
Build logic in AetherStudioZ is contributed, not hard-coded. The native Java and Android pipelines are
...
- `build-api` on the compile classpath. In-tree that is `implementation(project(":build-api"))`; out of tree
  it is a published coordinate (`io.github.aetherstudioz:build-api`, versioned through `plugin-bom`), declared
...
Note the name collision: `dev.aetherstudioz.plugin.Plugin` is the plugin SPI, and `dev.aetherstudioz.build.Plugin` is the
...
```kotlin
dependencies {
    compileOnly(platform("io.github.aetherstudioz:plugin-bom:<version>"))
    compileOnly("io.github.aetherstudioz:plugin-api")
    compileOnly("io.github.aetherstudioz:build-api")   // brings project-model-api, vfs-api, platform-core
}
```
...
names that resource: applying the Crashlytics **Gradle plugin** is what normally writes it, and AetherStudioZ has
...
| --- | --- | --- | --- |
| `dev.aetherstudioz.build.BUILD_PLUGIN_EP` | `platform.buildPlugin` | `BuildPlugin` | Tasks on every build graph |
| `dev.aetherstudioz.build.SOURCE_GENERATOR_EP` | `platform.sourceGenerator` | `SourceGenerator` | Generated source before compilation |
| `dev.aetherstudioz.build.RUN_TASK_PROVIDER_EP` | `platform.runTaskProvider` | `RunTaskProvider` | Rows in the Run picker |
| `dev.aetherstudioz.build.BUILD_SYSTEM_EP` | `platform.buildSystem` | `BuildSystem` | A whole pipeline |
| `dev.aetherstudioz.model.sync.PROJECT_IMPORTER_EP` | `platform.projectImporter` | `ProjectImporter` | Reading a foreign project model |
| `dev.aetherstudioz.model.sync.BUILD_FILE_WRITER_EP` | `platform.buildFileWriter` | `BuildFileWriter` | Writing declarations back to build files |
| `dev.aetherstudioz.model.FACET_CODEC_EP` | `platform.facetCodec` | `FacetCodec` | Decoding a facet an importer emits |
| `dev.aetherstudioz.model.ModuleTypeExtensionPoint` | `platform.moduleType` | `ModuleType` | A module type an importer can name |
| `dev.aetherstudioz.build.KOTLIN_COMPILER_PLUGIN_EP` | `platform.kotlinCompilerPlugin` | `KotlinCompilerPlugin` | A bytecode transformer inside kotlinc |
