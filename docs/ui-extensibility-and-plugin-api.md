...
> too, through a second, narrower published SPI (`dev.aetherstudioz.plugin.ui.UiPlugin` in `:plugin-ui-api`, named by
...
```kotlin
package dev.aetherstudioz.ui.api
...
| Top-level screens (`Screen` enum) | `AetherStudioZApp.kt` | `ScreenRegistry`; core screens built-in; routing reads the registry |
...
*Delivered (installment 1):* The **UI-side action registry** in `dev.aetherstudioz.ui.ext` (`UiActionHost`,
...
*Delivered (installment 2):* the **Compose-bearing registries** in `dev.aetherstudioz.ui.ext` — `ToolWindowRegistry`
...
installed plugin** (`uiEntryPoints` → `dev.aetherstudioz.plugin.ui.UiPlugin` from the published `:plugin-ui-api`,
...
`dev.aetherstudioz.core.event.IdeEventTopics` — editor (open/close/active/selection), build, run, analysis diagnostics,
...
*Stage 2 (done):* `IdeServicesBackend` is split into 13 per-service impl classes (`dev.aetherstudioz.core.backend.*`)
