...
// which buildSrc cannot do. Currently: `dev.aetherstudioz.kotlinc-art`, the Kotlin-compiler-on-ART ASM
// instrumentation.
plugins {
    `kotlin-dsl`
}
...
// The `dev.aetherstudioz.kotlinc-art` plugin id → :ide-android applies it with one line.
gradlePlugin {
    plugins {
        create("kotlincArt") {
            id = "dev.aetherstudioz.kotlinc-art"
            implementationClass = "dev.aetherstudioz.build.kotlinc.KotlincArtPlugin"
        }
    }
}
