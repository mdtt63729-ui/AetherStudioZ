package dev.aetherstudioz.lang.kotlin.resolve
            ...
            is KtClassOrObject -> {
                val fqn = node.fqName?.asString()
                    ?: (node as? org.jetbrains.kotlin.psi.KtObjectDeclaration)?.let {
                        dev.aetherstudioz.lang.kotlin.symbols.SourceIndexBuilder.localTypeFqn(
                            it
                        )
                    }
                    ?: (node as? KtClass)?.takeIf { it !is org.jetbrains.kotlin.psi.KtEnumEntry }
                        ?.let { dev.aetherstudioz.lang.kotlin.symbols.SourceIndexBuilder.localTypeFqn(it) }
        ...
        if (node is KtBlockExpression) {
            for (st in node.statements) {
                if (st is KtClassOrObject && st !is org.jetbrains.kotlin.psi.KtEnumEntry &&
                    ...
                    st.name!!,
                    dev.aetherstudioz.lang.kotlin.symbols.SourceIndexBuilder.localTypeFqn(st)
                )
            }
        }
        node = node.parent
