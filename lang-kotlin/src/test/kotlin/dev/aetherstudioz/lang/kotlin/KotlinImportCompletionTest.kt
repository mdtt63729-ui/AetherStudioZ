...
class KotlinImportCompletionTest {
    ...
    fun topLevelDeclarationsAreImportableAfterAPackageDot() {
        // The reported case: `fun Test()` (a top-level function) in `com.aetherstudioz.test` was NOT offered at
        // `import com.aetherstudioz.test.<caret>` — package-member completion listed only types. Top-level functions AND
        // properties declared in the package are importable by name, so both must appear.
        val srcDir = tempProject(
            mapOf(
                "test/Api.kt" to "package com.aetherstudioz.test\nfun Test() {}\nval flag = true",
                "Use.kt" to "package other\n",
            ),
        )
        val analyzer = KotlinSourceAnalyzer(fakeContext(srcDir))
        val items = runBlocking { analyzer.completeAtCaret(srcDir, "Use.kt", "package other\nimport com.aetherstudioz.test.|") }.items
