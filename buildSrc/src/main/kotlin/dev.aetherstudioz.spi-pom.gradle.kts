...
//
// Applied by `dev.aetherstudioz.spi-publish` for the SPI jars, and directly by `:plugin-bom`, which is a
...
val projectUrl = "https://github.com/aetherstudioz/AetherStudioZ"
...
group = "io.github.aetherstudioz"
version = spiVersion

publishing {
    publications.withType<MavenPublication>().configureEach {
        pom {
            name.set("AetherStudioZ ${project.name}")
            description.set(
                when (project.name) {
                    "plugin-api" ->
                        "The AetherStudioZ plugin SPI: the Plugin entry point, its manifest, the " +
                            "registration API, and the action model."
                    "plugin-ui-api" ->
                        "The AetherStudioZ plugin UI SPI: what a plugin implements to contribute a tool " +
                            "window, a screen, or an overlay, and the context those bodies render against."
                    "platform-core" ->
                        "The AetherStudioZ platform substrate the plugin SPI exposes: extension points, " +
                            "scoped services, the message bus, logging, and the settings model."
                    "project-model-api" ->
                        "The AetherStudioZ project model: module types, project templates, facets and " +
                            "their codecs, and file icons."
                    "language-api" ->
                        "The AetherStudioZ language SPI: the neutral syntax tree, resolution, file types, " +
                            "completion, formatting, folding, and a language's compilation context."
                    "analysis-api" ->
                        "The AetherStudioZ analysis SPI: analyzers, the diagnostic model, quick fixes, and " +
                            "editor action providers."
                    "index-api" ->
                        "The AetherStudioZ index SPI: persisted, incrementally maintained project indexes."
                    "build-api" ->
                        "The AetherStudioZ build SPI: build systems, build plugins, source generators, and " +
                            "run tasks."
                    "vfs-api" ->
                        "The AetherStudioZ virtual file system, which the other SPI artifacts expose."
                    "plugin-bom" ->
                        "Versions for everything a AetherStudioZ plugin compiles against: the SPI artifacts " +
                            "and the Compose the IDE provides at runtime."
                    else -> "A AetherStudioZ plugin SPI artifact."
                }
            )
            url.set(projectUrl)
            licenses {
                license {
                    ...
                    comments.set(
                        "AetherStudioZ is GPL-3.0-or-later. This module adds the Classpath exception so a " +
            ...
            developers {
                developer {
                    id.set("aetherstudioz")
                    name.set("aetherstudioz")
                    url.set("https://github.com/aetherstudioz")
                }
            }
            scm {
                url.set(projectUrl)
                connection.set("scm:git:$projectUrl.git")
                developerConnection.set("scm:git:ssh://git@github.com/aetherstudioz/AetherStudioZ.git")
            }
            issueManagement {
