...
/**
 * The standalone stdio entry point for the AetherStudioZ MCP server.
 *
 * Usage:
 * ```
 ...
 * rooted at `--project` (defaulting to the current directory or `$AETHERSTUDIOZ_MCP_PROJECT`), so a client
...
fun main(args: Array<String>) {
    var project = System.getenv("AETHERSTUDIOZ_MCP_PROJECT")
    ...
    while (i < args.size) {
        when (args[i]) {
            ...
            "--http" -> {
                ...
                httpPort = next?.toIntOrNull() ?: AetherStudioZMcpServer.DEFAULT_HTTP_PORT
                if (next?.toIntOrNull() != null) i++
            }
            "--ftp" -> {
                val next = args.getOrNull(i + 1)
                ftpPort = next?.toIntOrNull() ?: AetherStudioZMcpServer.DEFAULT_FTP_PORT
                if (next?.toIntOrNull() != null) i++
            }
            "--help", "-h" -> {
                println(
                    ...
                    |                   stdio. Defaults to port ${'$'}{AetherStudioZMcpServer.DEFAULT_HTTP_PORT}.
                    ...
                    |                   ${'$'}{AetherStudioZMcpServer.DEFAULT_FTP_PORT}) rooted at <project>/assets.
    ...
    if (httpPort != null || ftpPort != null) {
        ...
        if (httpPort != null) {
            val server = AetherStudioZMcpServer.startHttpServer(workspace, port = httpPort, gate = gate)
            System.err.println("AetherStudioZ MCP server ready over HTTP (project: $root, port: ${server.port})")
    ...
    System.err.println("AetherStudioZ MCP server ready (project: $root)")
    ...
    val server = AetherStudioZMcpServer.build(transport, workspace, gate = gate, mapper = mapper)
...
private fun startFtpServer(root: Path, port: Int): FtpServer {
    ...
    val server = AetherStudioZMcpServer.startFtpServer(assets, port)
