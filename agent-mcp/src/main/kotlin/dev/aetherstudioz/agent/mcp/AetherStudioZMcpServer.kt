...
/**
 * Turns AetherStudioZ's agent tool layer into an MCP server. Every tool in the registry is advertised over the
 ...
 * resources expose the project's live overview and agent memory, and a prompt returns the AetherStudioZ agent
 ...
 */
object AetherStudioZMcpServer {

    const val DEFAULT_SERVER_NAME = "aetherstudioz"
    const val DEFAULT_SERVER_VERSION = "1.0.0"

    const val PROJECT_OVERVIEW_URI = "aetherstudioz://project/overview"
    const val PROJECT_MEMORY_URI = "aetherstudioz://project/memory"
    const val AGENT_PROMPT = "aetherstudioz_agent"
    ...
    /**
     * A `aetherstudioz_agent` prompt that returns the same grounding the in-IDE agent is given, so a client
    ...
    private fun agentPrompt(
        ...
        val prompt = McpSchema.Prompt.builder(AGENT_PROMPT)
            .description("The AetherStudioZ agent grounding: what this IDE is and how to work in it.")
            .build()
        return McpServerFeatures.SyncPromptSpecification(prompt) { _, _ ->
            ...
            McpSchema.GetPromptResult.builder(messages)
                .description("AetherStudioZ agent grounding and working rules.")
                .build()
        }
    }
}
