...
import dev.aetherstudioz.agent.DiagnosticInfo
import dev.aetherstudioz.agent.ModuleInfo
...
import dev.aetherstudioz.agent.SymbolHit
import dev.aetherstudioz.agent.TextEdit
import dev.aetherstudioz.agent.TextMatch
import dev.aetherstudioz.agent.WorkspaceEntry
 ...
 */
class AetherStudioZMcpServerTest {
    ...
    fun `initialize and tools list expose the agent tool set`() {
        ...
        try {
            ...
            assertEquals(AetherStudioZMcpServer.DEFAULT_SERVER_NAME, init.serverInfo().name())
    ...
    fun `resources and prompt are advertised`() {
        ...
        try {
            ...
            assertTrue(AetherStudioZMcpServer.PROJECT_OVERVIEW_URI in resourceUris)
            assertTrue(AetherStudioZMcpServer.PROJECT_MEMORY_URI in resourceUris)

            val memory = client.readResource(
                McpSchema.ReadResourceRequest.builder(AetherStudioZMcpServer.PROJECT_MEMORY_URI).build(),
            )
            ...
            val prompt = client.getPrompt(
                McpSchema.GetPromptRequest.builder(AetherStudioZMcpServer.AGENT_PROMPT).build(),
            )
            ...
            assertTrue("AetherStudioZ" in body)
    ...
    private fun connect(workspace: AgentWorkspace, gate: AgentPermissionGate): Pair<Server, Client> {
        ...
        val server = AetherStudioZMcpServer.build(
