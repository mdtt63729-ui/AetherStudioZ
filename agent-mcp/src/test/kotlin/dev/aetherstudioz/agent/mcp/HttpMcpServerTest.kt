...
class HttpMcpServerTest {
    ...
    fun `streamable HTTP handshake and tool calls work end to end`() {
        val fake = AetherStudioZMcpServerTest.FakeWorkspace("src/Main.kt" to "fun main() { println(\"hi\") }")
        val server = AetherStudioZMcpServer.startHttpServer(
            workspace = fake,
            port = 0,
    ...
    fun `second client gets its own session`() {
        val fake = AetherStudioZMcpServerTest.FakeWorkspace("a.txt" to "a")
        val server = AetherStudioZMcpServer.startHttpServer(fake, port = 0)
    ...
    fun `raw HTTP wire format matches the streamable transport contract`() {
        val fake = AetherStudioZMcpServerTest.FakeWorkspace()
        val server = AetherStudioZMcpServer.startHttpServer(fake, port = 0)
        ...
        try {
            ...
            assertTrue(init.body().contains("\"aetherstudioz\""))
    ...
    fun `a body with non-ASCII text is framed by bytes, not chars`() {
        val fake = AetherStudioZMcpServerTest.FakeWorkspace("a.kt" to "val greeting = \"hi\"")
        val server = AetherStudioZMcpServer.startHttpServer(fake, port = 0)
        val http = java.net.http.HttpClient.newHttpClient()
        try {
            val base = "http://127.0.0.1:${server.port}/mcp"
    ...
    fun `a foreign Origin is refused so a rebound hostname cannot drive the server`() {
        val server = AetherStudioZMcpServer.startHttpServer(AetherStudioZMcpServerTest.FakeWorkspace(), port = 0)
        val http = java.net.http.HttpClient.newHttpClient()
        try {
            val base = "http://127.0.0.1:${server.port}/mcp"
