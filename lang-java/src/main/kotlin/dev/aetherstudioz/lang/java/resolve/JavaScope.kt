package dev.aetherstudioz.lang.java.resolve
...
class JavaScope(
     ...
     *  off ([dev.aetherstudioz.lang.java.env.JavaRecordSupport]): when it is ON, the real backing field is already offered
    ...
    override fun resolve(name: String): ResolveResult {
        ...
        dev.aetherstudioz.psi.IntellijPsiHost.withParseLock { facade.findClass(name, GlobalSearchScope.allScope(project)) }
    ...
    private fun applyFilter(symbols: List<Symbol>, filter: SymbolFilter): List<Symbol> {
        ...
        return symbols.filter { s ->
            (kinds == null || s.kind in kinds) &&
                (!filter.staticOnly || dev.aetherstudioz.lang.resolve.Modifier.STATIC in s.modifiers) &&
                (!filter.instanceOnly || dev.aetherstudioz.lang.resolve.Modifier.STATIC !in s.modifiers)
        }
    }
}
