package dev.aetherstudioz.lang.jdt.completion
...
import dev.aetherstudioz.index.PackageTypesIndex
import dev.aetherstudioz.index.PackagesIndex
import dev.aetherstudioz.index.exactAll
import dev.aetherstudioz.index.fuzzyAll
...
internal object CandidateCollector {
    ...
    fun collect(
        ...
        docs: dev.aetherstudioz.lang.jdt.SourceMethodResolver? = null,
    ...
    private fun members(ctx: AnalyzedContext, appendCallParens: Boolean, docs: dev.aetherstudioz.lang.jdt.SourceMethodResolver?): List<Candidate> {
    ...
    private fun names(ctx: AnalyzedContext, appendCallParens: Boolean, docs: dev.aetherstudioz.lang.jdt.SourceMethodResolver?): List<Candidate> {
     ...
     */
    private fun methodCandidate(m: MethodBinding, from: ReferenceBinding?, appendCallParens: Boolean, discouraged: Boolean = false, docs: dev.aetherstudioz.lang.jdt.SourceMethodResolver? = null): Candidate {
        ...
        return Candidate(
            ...
            documentation = info?.javadocRaw?.let { dev.aetherstudioz.lang.jdt.JavadocText.clean(it) }?.takeIf { it.isNotEmpty() },
        )
    }
    ...
    private var lastMatcher: dev.aetherstudioz.lang.completion.PrefixMatcher? = null

    private fun matches(name: String, prefix: String): Boolean {
        if (prefix.isEmpty()) return true
        val m = lastMatcher?.takeIf { it.prefix == prefix }
            ?: dev.aetherstudioz.lang.completion.PrefixMatcher(prefix).also { lastMatcher = it }
