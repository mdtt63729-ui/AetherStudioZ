...
import dev.aetherstudioz.ui.ads.AdController
import dev.aetherstudioz.ui.backend.UiProjectFolderKind
import dev.aetherstudioz.ui.backend.AdHost
...
import dev.aetherstudioz.ui.platform.ioDispatcher
import dev.aetherstudioz.ui.screens.ModulesTab
import dev.aetherstudioz.ui.screens.doImportGradle
import dev.aetherstudioz.ui.theme.CaAccent
 ...
 *
 * Held outside the composition (see [rememberAetherStudioZAppState]) so the root composable stays a pure
...
class AetherStudioZAppState(
     ...
     * AetherStudioZ workspace. Asking first (as this did) meant a AetherStudioZ project folder had to answer a
     ...
     */
    fun openNotificationTarget(notification: dev.aetherstudioz.ui.backend.UiNotification) {
        when (val target = notification.target) {
            null -> Unit
            is dev.aetherstudioz.ui.backend.UiNotificationTarget.Project -> {
                val match = backend.projects.projects().firstOrNull { it.rootPath == target.rootPath }
                if (match != null) openProject(match)
            }
            is dev.aetherstudioz.ui.backend.UiNotificationTarget.StoreItem -> {
                scope.launch {
            ...
            dev.aetherstudioz.ui.backend.UiNotificationTarget.Submissions -> openSubmitProject()
            is dev.aetherstudioz.ui.backend.UiNotificationTarget.Screen -> {
...
/**
 * Creates (and remembers) the [AetherStudioZAppState] for this composition, tied to a scope that is cancelled
 * when the app leaves it. Re-created only if the backend or a host bridge is swapped.
 */
@Composable
fun rememberAetherStudioZAppState(
    backend: IdeBackend,
    fileActions: FileActions = FileActions.None,
    adHost: AdHost = AdHost.None,
    scope: CoroutineScope = rememberCoroutineScope(),
): AetherStudioZAppState = remember(backend, fileActions, adHost, scope) {
    AetherStudioZAppState(backend, fileActions, scope, adHost)
}
...
internal fun accentOf(settings: UiSettings): CaAccent = when (settings.accent) {
    ...
    UiAccent.Indigo -> CaAccent.Indigo
    else -> CaAccent.Lime
}
