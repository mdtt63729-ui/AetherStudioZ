...
import dev.aetherstudioz.ui.platform.dynamicColorSchemeOrNull

/**
 * The AetherStudioZ design system, now built on **Material 3 Expressive**. [AetherStudioZTheme] installs a real
 ...
 * [AetherStudioZColors] / [CaTypography]); its generic fields are M3-derived from the active [ColorScheme] via
 ...
 */
enum class CaAccent { Lime, Violet, Teal, Orange, Indigo }
...
@Immutable
data class AetherStudioZColors(
    val isDark: Boolean,
    // surfaces
    val bg: Color,
    val editorBg: Color,
...
private fun darkColors(accent: Color, accentStrong: Color) = AetherStudioZColors(
    isDark = true,
    bg = Color(0xFF131317),
    editorBg = Color(0xFF171821),
    consoleBg = Color(0xFF1F1F25),
    surface = Color(0xFF131317),
    surface2 = Color(0xFF29292F),
    surface3 = Color(0xFF34343A),
    textPrimary = Color(0xFFE4E1E9),
    textSecondary = Color(0xFFC6C5D0),
    textTertiary = Color(0xFF90909A),
    ...
    glassThin = Color(0xFF1B1B21).copy(alpha = 0.55f),
    glassReg = Color(0xFF1F1F25).copy(alpha = 0.72f),
    glassThick = Color(0xFF131317).copy(alpha = 0.86f),
...
private fun lightColors(accent: Color, accentStrong: Color) = AetherStudioZColors(
    isDark = false,
    bg = Color(0xFFDBD9E0),
    editorBg = Color(0xFFFFFFFF),
    consoleBg = Color(0xFFF4F2FA),
    surface = Color(0xFFFBF8FF),
    surface2 = Color(0xFFEFEDF4),
    surface3 = Color(0xFFE3E1E9),
    textPrimary = Color(0xFF1B1B21),
    textSecondary = Color(0xFF45464F),
    textTertiary = Color(0xFF767680),
    textOnAccent = Color(0xFFFFFFFF),
...
fun caColors(dark: Boolean, accent: CaAccent): AetherStudioZColors = when {
    ...
    // Legacy AetherStudioZ orange (the Darcula `#CC7832` from the classic `<>` logo).
    ...
    // Aether Blue: the editor's caret/selection reads as a soft periwinkle on dark and a deep
    // blue on light — matching the AetherStudioZ design system.
    dark && accent == CaAccent.Indigo -> darkColors(Color(0xFFAEC6FF), Color(0xFF9DB8F5))
    !dark && accent == CaAccent.Indigo -> lightColors(Color(0xFF3A5B9F), Color(0xFF2E4A80))
...
@Immutable
object Radius {
    val xs = 4.dp; val sm = 8.dp; val control = 12.dp; val md = 12.dp
    val lg = 16.dp; val xl = 28.dp; val sheet = 28.dp; val pill = 999.dp
}
...
val LocalCaColors = staticCompositionLocalOf<AetherStudioZColors> { error("AetherStudioZTheme not applied") }
val LocalCaType = staticCompositionLocalOf<CaTypography> { error("AetherStudioZTheme not applied") }

/** Token accessor: `Ca.colors`, `Ca.type`, `Ca.spacing`, `Ca.radius`. */
object Ca {
    val colors: AetherStudioZColors
...
/**
 * Bridge the bespoke [AetherStudioZColors] onto the active M3 [ColorScheme]: the generic chrome fields
 ...
 */
private fun AetherStudioZColors.bridgedTo(scheme: ColorScheme): AetherStudioZColors = copy(
...
private fun AetherStudioZColors.toIdeColors(): IdeColors = IdeColors(
    isDark = isDark,
...
@Composable
fun AetherStudioZTheme(
    dark: Boolean = true,
    accent: CaAccent = CaAccent.Indigo,
