...
import dev.aetherstudioz.preview.FontMetrics
...
internal class ComposePaint : RPaint {
    ...
    override var gradient: dev.aetherstudioz.preview.Gradient? = null
}
...
internal class ComposeRCanvas(
...
) : RCanvas, dev.aetherstudioz.preview.NativeCanvasHolder {
    ...
    private fun paintFor(p: RPaint, l: Float, t: Float, r: Float, b: Float): Paint = paint(p).also {
        ...
        it.shader = when (g.type) {
            dev.aetherstudioz.preview.GradientType.RADIAL -> androidx.compose.ui.graphics.RadialGradientShader(
                center = Offset(l + (r - l) * g.centerX, t + (b - t) * g.centerY),
                radius = minOf(r - l, b - t) * g.radiusFraction,
                colors = colors,
            )
            dev.aetherstudioz.preview.GradientType.SWEEP -> androidx.compose.ui.graphics.SweepGradientShader(
