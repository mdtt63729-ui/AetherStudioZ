...
class PersistenceRoundTripTest {
    ...
    @Test
    fun threeModuleProjectSavesAndReloadsToIdenticalSnapshot() = withTempDir("aetherstudioz-roundtrip") { dir ->
    ...
    @Test
    fun variantQualifiedDependenciesRoundTrip() = withTempDir("aetherstudioz-variant-deps") { dir ->
        val platform = PlatformCore()
        platform.registerTestTypes()
        try {
            val store = ProjectModel.open(dir, platform, FacetCodecRegistry().register(JavaFacetCodec))
    ...
    @Test
    fun addAndRemoveContentRootRoundTrips() = withTempDir("aetherstudioz-roots") { dir ->
        val platform = PlatformCore()
        platform.registerTestTypes()
        try {
            val store = ProjectModel.open(dir, platform, FacetCodecRegistry().register(JavaFacetCodec))
