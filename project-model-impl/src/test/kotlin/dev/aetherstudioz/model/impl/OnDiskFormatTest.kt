...
class OnDiskFormatTest {
    ...
    @Test
    fun writesReadableWorkspaceJsonAndModuleToml() = withTempDir("aetherstudioz-format") { dir ->
    ...
    @Test
    fun variantQualifiedDependenciesUseNestedTables() = withTempDir("aetherstudioz-variant-format") { dir ->
        val platform = PlatformCore()
        platform.registerTestTypes()
        try {
            val store = ProjectModel.open(dir, platform, FacetCodecRegistry().register(JavaFacetCodec))
