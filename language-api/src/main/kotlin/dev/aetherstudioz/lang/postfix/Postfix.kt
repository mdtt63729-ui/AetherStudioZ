package dev.aetherstudioz.lang.postfix
 ...
 * [dev.aetherstudioz.lang.completion.CompletionContributor] collects the applicable ones at a `MEMBER_ACCESS` position
 * and surfaces each as an ordinary `CompletionItem(kind = SNIPPET)` whose `additionalEdits` delete the
 * `expr.` receiver span and whose `caret` is a [dev.aetherstudioz.lang.completion.CaretAction.ExpandSnippet] driving
...
interface PostfixTemplate {
    ...
    /** Languages this template applies to (by [dev.aetherstudioz.lang.LanguageId]); empty = every language. A template
     *  emitting language-specific syntax (a Kotlin `?.let { }`, a Java `;`) scopes itself so the driver doesn't
     *  offer it in the wrong file. */
    val languages: Set<dev.aetherstudioz.lang.LanguageId> get() = emptySet()
 ...
 * `for`), to be driven via [dev.aetherstudioz.lang.completion.CaretAction.ExpandSnippet].
 */
