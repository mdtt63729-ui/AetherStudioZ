package dev.aetherstudioz.store.impl

import dev.aetherstudioz.store.StoreProvider
import dev.aetherstudioz.store.StoreResult
...
class SupabaseAccountServiceTest {
    ...
    fun readsPkceCodeFromTheQueryString() {
        val p = SupabaseAccountService.redirectParams("aetherstudioz://auth-callback?code=abc123&state=xyz")
    ...
    fun readsImplicitTokensFromTheFragment() {
        val p = SupabaseAccountService.redirectParams(
            "aetherstudioz://auth-callback#access_token=head.body.sig&refresh_token=r-123&expires_in=3600&token_type=bearer",
        )
    ...
    fun mergesQueryAndFragment() {
        val p = SupabaseAccountService.redirectParams("aetherstudioz://cb?state=s1#access_token=t1&refresh_token=r1")
    ...
    fun urlDecodesValues() {
        val p = SupabaseAccountService.redirectParams("aetherstudioz://cb?error_description=Email%20not%20confirmed")
        assertEquals("Email not confirmed", p["error_description"])
    }

    @Test
    fun emptyAndMalformedRedirectsYieldNothingRatherThanThrowing() {
        assertTrue(SupabaseAccountService.redirectParams("aetherstudioz://auth-callback").isEmpty())
        ...
        assertTrue(SupabaseAccountService.redirectParams("aetherstudioz://cb?&&").isEmpty())
    }
    ...
    fun providerErrorOnTheRedirectBecomesAFailure() {
        val svc = service()
        val r = svc.complete("aetherstudioz://auth-callback?error=access_denied&error_description=User%20cancelled")
    ...
    fun redirectWithNeitherCodeNorTokenIsAFailure() {
        val r = service().complete("aetherstudioz://auth-callback?state=only")
    ...
    fun authorizeUrlNamesTheProviderAndEncodesTheRedirect() {
        ...
        assertTrue(url.contains("redirect_to=aetherstudioz%3A%2F%2Fauth-callback"), url)
    }
    ...
    fun unconfiguredServiceOffersNothingAndDoesNotThrow() {
        ...
        assertTrue(svc.complete("aetherstudioz://cb?code=x") is StoreResult.Unavailable)
    ...
    fun signOutClearsTheStoredRefreshToken() {
        ...
        val svc = SupabaseAccountService(
            url = "https://example.invalid", apiKey = "k", redirectUrl = "aetherstudioz://cb", tokens = store,
        )
        svc.signOut()
    ...
    private fun service() = SupabaseAccountService(
        ...
        redirectUrl = "aetherstudioz://auth-callback",
    )
}
