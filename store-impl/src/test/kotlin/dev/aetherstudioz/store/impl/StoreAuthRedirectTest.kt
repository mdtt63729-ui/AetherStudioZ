...
import dev.aetherstudioz.store.StoreAuth
...
class StoreAuthRedirectTest {
    ...
    fun recognisesTheRedirectTheProviderSendsBack() {
        ...
        assertTrue(StoreAuth.isAuthRedirect("aetherstudioz://auth-callback#access_token=abc&refresh_token=d"))
        // PKCE: a code in the query instead.
        assertTrue(StoreAuth.isAuthRedirect("aetherstudioz://auth-callback?code=abc"))
        // Bare, and with a trailing slash.
        assertTrue(StoreAuth.isAuthRedirect("aetherstudioz://auth-callback"))
        assertTrue(StoreAuth.isAuthRedirect("aetherstudioz://auth-callback/"))
        // Schemes are case-insensitive per RFC 3986, and some browsers normalise them.
        assertTrue(StoreAuth.isAuthRedirect("AETHERSTUDIOZ://auth-callback#access_token=abc"))
    }
    ...
    fun doesNotClaimIntentsThatBelongToTheFileImporter() {
        ...
        assertFalse(StoreAuth.isAuthRedirect("aetherstudioz://open-project?path=/x"))
...
class StoreAuthErrorMessageTest {
    ...
    private fun completeWith(errorDescription: String): String {
        ...
        val result = service.complete(
            "${dev.aetherstudioz.store.StoreAuth.ANDROID_REDIRECT}#error=server_error&error_description=$encoded",
        )
        return (result as dev.aetherstudioz.store.StoreResult.Failed).message
    }
