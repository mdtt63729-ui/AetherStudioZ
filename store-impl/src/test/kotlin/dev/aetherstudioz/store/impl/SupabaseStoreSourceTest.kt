...
class SupabaseStoreSourceTest {
    ...
    private val realCatalogJson = """
    {
      ...
      "featured": [
        {"id":"sample-calculator","kind":"sample","title":"Calculator",
         ...
         "author":"AetherStudioZ","featured":true,"installs":0,"ratingCount":0,
    ...
    fun parsesTheRealCatalogDocument() {
        val c = SupabaseStoreSource.parseCatalog(dev.aetherstudioz.platform.JsonReader.parse(realCatalogJson))
    ...
    fun templateItemHasNoPayloadAndKeepsItsTemplateId() {
        val calculator = SupabaseStoreSource.parseCatalog(dev.aetherstudioz.platform.JsonReader.parse(realCatalogJson)).featured.single()
    ...
    fun malformedRowsAreDroppedNotFatal() {
        ...
        val c = SupabaseStoreSource.parseCatalog(dev.aetherstudioz.platform.JsonReader.parse(json))
    ...
    fun jsonStringEscapingHandlesQuotesAndControlChars() {
        ...
        val roundTripped = dev.aetherstudioz.platform.JsonReader.parse("{\"q\":" + SupabaseStoreSource.jsonStr(nasty) + "}")
        assertEquals(nasty, dev.aetherstudioz.platform.JsonReader.str(roundTripped, "q"))
    }
}
