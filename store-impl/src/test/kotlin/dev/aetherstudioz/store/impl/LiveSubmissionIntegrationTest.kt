...
import dev.aetherstudioz.store.StoreAccount
...
class LiveSubmissionIntegrationTest {
    ...
    private fun userId(email: String): String? = runCatching {
        ProcessBuilder(
            "docker", "exec", "supabase_db_aetherstudioz", "psql", "-U", "postgres", "-tAc",
    ...
    private fun accountsFor(uid: String): SupabaseAccountService {
        val svc = SupabaseAccountService(baseUrl, anonKey, "aetherstudioz://auth-callback")
        ...
        val r = svc.complete("aetherstudioz://auth-callback#access_token=${mintJwt(uid)}&refresh_token=none")
    ...
    private fun deleteTestItems(email: String) {
        runCatching {
            ProcessBuilder(
                "docker", "exec", "supabase_db_aetherstudioz", "psql", "-U", "postgres", "-c",
                """
    ...
    fun packagesAndSubmitsAgainstTheRealBackend() {
        ...
        val request = dev.aetherstudioz.store.StoreSubmissionRequest(
    ...
    private fun screenshotPathsOf(slug: String, version: String): List<String> {
        val out = ProcessBuilder(
            "docker", "exec", "supabase_db_aetherstudioz", "psql", "-U", "postgres", "-At", "-c",
            """
    ...
    fun pendingQuotaMessageSurfacesFromTheTrigger() {
        ...
        for (i in 1..12) {
            val r = service.submit(
                dev.aetherstudioz.store.StoreSubmissionRequest(
