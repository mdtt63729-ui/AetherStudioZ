...
import dev.aetherstudioz.store.RemoteReviewPage
...
class SupabaseReviewService(
    ...
    override fun report(
        itemSlug: String,
        authorId: String,
        reason: dev.aetherstudioz.store.ReportReason,
        detail: String?,
    ...
    override fun publisherProfile(handle: String): StoreResult<dev.aetherstudioz.store.RemotePublisherProfile?> =
    ...
    companion object {
        ...
        internal fun parseProfile(root: Any?): dev.aetherstudioz.store.RemotePublisherProfile? {
            val handle = JsonReader.str(root, "handle") ?: return null
            val avg = JsonReader.obj(root)?.get("averageRating")
            return dev.aetherstudioz.store.RemotePublisherProfile(
                handle = handle,
