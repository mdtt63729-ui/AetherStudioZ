...
open class Window : Container(), Surface {
    ...
    private fun findPopup(
        ...
        offsetX: Int,
        offsetY: Int,
        visit: (dev.aetherstudioz.swing.JComboBox<*>, Int, Int) -> Component?,
    ): Component? {
        for (child in container.components()) {
            if (!child.isVisible()) continue
            val x = offsetX + child.getX()
            val y = offsetY + child.getY()
            if (child is dev.aetherstudioz.swing.JComboBox<*> && child.isPopupVisible()) {
...
internal object NoCanvas : RCanvas {
    ...
    override fun drawImage(img: dev.aetherstudioz.preview.RImage, l: Float, t: Float, r: Float, b: Float, tintArgb: Int?) {}
