...
import dev.aetherstudioz.awt.BorderLayout
...
import dev.aetherstudioz.awt.Frame
import dev.aetherstudioz.awt.Graphics
import dev.aetherstudioz.awt.LayoutManager
 ...
 * ends the run (see `dev.aetherstudioz.awt.ToolkitWindows`), so the distinction that made `EXIT_ON_CLOSE` dangerous
...
object SwingUtilities {
    ...
    @JvmStatic
    fun getWindowAncestor(c: Component?): dev.aetherstudioz.awt.Window? {
        var node: Component? = c
        while (node != null) {
            if (node is dev.aetherstudioz.awt.Window) return node
            node = node.parent
        }
        return null
    }
