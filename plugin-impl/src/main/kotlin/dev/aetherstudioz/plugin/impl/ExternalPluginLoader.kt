...
class ExternalPluginLoader(
    ...
    sealed interface Result {
         ...
         * other facets from it. The UI facet (`dev.aetherstudioz.plugin.ui.UiPlugin`) is loaded that way: its type lives
    ...
    fun load(discovered: DiscoveredPlugin): Result {
        ...
        if (!PluginVersions.satisfies(hostVersion, min)) {
            return Result.Failed(manifest, "requires AetherStudioZ $min or newer")
        }
