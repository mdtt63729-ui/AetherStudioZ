package dev.aetherstudioz.ui.backend
 ...
 * [dev.aetherstudioz.ui.AetherStudioZApp]; [None] (the default) means "this platform has no ads".
 *
 * Ad slots are NATIVE — the host renders them inside the app's own card chrome (see
 * [dev.aetherstudioz.ui.components.AdSlot]), never as banners. The one exception is a single full-screen interstitial
 ...
 * that interstitial — is decided in common by [dev.aetherstudioz.ui.ads.AdController] from the user's "show ads"
...
interface AdHost {
     ...
     * [dev.aetherstudioz.ui.ads.AdController] uses it to turn ads back on once per install/update. Null where the host
     ...
     * are active (see [dev.aetherstudioz.ui.ads.AdController]); the host loads/caches the ad and paints it to match the
     * app. A host with no ad ready may render nothing — the surrounding [dev.aetherstudioz.ui.components.AdSlot] then
