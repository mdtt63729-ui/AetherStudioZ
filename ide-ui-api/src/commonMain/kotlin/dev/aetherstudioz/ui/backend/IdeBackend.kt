...
interface IdeBackend {
     ...
     */
    fun uiPlugins(): List<dev.aetherstudioz.ui.ext.UiPlugin> = emptyList()
}
...
sealed interface UiContentBlock {
     ...
     * through the platform [dev.aetherstudioz.ui.ComposePreviewHost] (the same on-device/desktop interpreter the editor's
 ...
 * neither a AetherStudioZ workspace nor anything an importer claims. The files are open for editing, but the
...
/**
 * The result of converting a Gradle compatibility-mode project to a native AetherStudioZ project — or reverting
...
data class TreeNode(
     ...
     * directory — so a New-Class action can target any intermediate level (e.g. `com.aetherstudioz` inside a
     * collapsed `com.aetherstudioz.aetherstudioz`). Empty for non-package nodes.
     */
 ...
 */
enum class UiAccent { Dynamic, Lime, Violet, Teal, Orange, Indigo, Custom }
