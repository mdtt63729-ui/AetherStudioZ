...
import dev.aetherstudioz.android.support.tools.ToolResult
...
class DexArchiveCacheTest {
    ...
    fun duplicateLibraryOnClasspathDoesNotCrashDexing() = runBlocking {
        withTempDir("dexdup") { tmp ->
            ...
            val result = buildTask(":x:dexBuilder", listOf(dupA, dupB, verA, verB, other), extRoot, dexer, tmp.resolve("cache"), tmp.resolve("x"))
                .execute(SimpleTaskContext())
            assertEquals(dev.aetherstudioz.build.TaskResult.Success, result, "class-level dedup must keep duplicate types off the classpath")
    ...
    fun classFreeLibraryJarsAreSkippedNotDexedOrCrashed() = runBlocking {
        withTempDir("dexempty") { tmp ->
            ...
            val result = buildTask(":x:dexBuilder", listOf(zeroEntry, manifestOnly, normal), extRoot, dexer, tmp.resolve("cache"), tmp.resolve("x"))
                .execute(SimpleTaskContext())
            assertEquals(dev.aetherstudioz.build.TaskResult.Success, result, "class-free jars must not break the dex build")
    ...
    fun coldLibraryArchivingRoutesOffHeapWhenSupported() = runBlocking {
        withTempDir("dexoffheap") { tmp ->
            ...
            assertEquals(dev.aetherstudioz.build.TaskResult.Success, result)
    ...
    fun droppedLibraryClassFailsTheBuildLoudly() = runBlocking {
        withTempDir("dexdrop") { tmp ->
            ...
            assertTrue(result is dev.aetherstudioz.build.TaskResult.Failed, "a dropped class must fail the build, not ship a broken APK")
            val msg = (result as dev.aetherstudioz.build.TaskResult.Failed).message
