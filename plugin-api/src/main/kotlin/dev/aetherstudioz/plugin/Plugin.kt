 ...
 * the plugin never threads a [dev.aetherstudioz.platform.PluginId] by hand.
 *
 * Teardown is automatic: the [dev.aetherstudioz.platform.Disposable]s produced by `register` (plus a bulk
...
interface Plugin {
     ...
     * `res/raw/aetherstudioz_plugin.toml`, which the host reads before any of the plugin's code runs and uses in
     ...
     *
     * A **built-in** has no packaged manifest, so it must override this. [dev.aetherstudioz.plugin.PluginRegistration]
