 ...
 * `dev.aetherstudioz.spi-publish`, and `:plugin-bom` names it in a constraint. A tenth SPI module added without the
    ...
    fun `the BOM constrains every module this build publishes`() {
        ...
        val published = root.listFiles().orEmpty()
            ...
            .filter { module ->
                ...
                buildFile.isFile && "dev.aetherstudioz.spi-publish" in buildFile.readText()
            }
            .map { it.name }
            .sorted()
